...
- Consult `knowledge-mentor` before designing a flow or interaction pattern with no existing precedent among the already-approved docs in `product/02-ux/` — state the specific question and request the consultation before finalizing that part of the design, rather than inventing the pattern from scratch (load `consultation-pattern` for the mechanic). The design decision stays yours.
