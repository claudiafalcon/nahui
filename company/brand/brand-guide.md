...
## Cards
- Stat/highlight card (e.g. "Ventas del día $4,850.00") — solid Coral background, white text, large number emphasis. **Resolved:** uses Coral AA+ (`#C13F26`, 5.24:1), the same accessible token adopted for the Primary button under Q12 — confirmed in every built instance (Home's running-total header, Resultados' hero totals, Session/Event detail, Home's close-summary), 2026-08-04.
