...
**Status: executed by the Product Owner, 2026-08-17-20 (MXN $500/3 days, single static ad, single audience/entry point).** This document's plan ran as designed; the Google Form response data it produced is what motivated `company/merchant-validation-strategy-v2.md` (2026-08-18). This document's own content below is left as the historical record of what was actually planned and run — not rewritten in hindsight.

**Retirement note, added 2026-09-10 (Product Owner decision).** This document's own historical-record status above is unchanged — nothing below is rewritten, since it already correctly describes itself as a closed, executed run (2026-08-17–20), not an active plan.

Added here specifically for anyone reading this document as a template for a future campaign: its Website URL destination (`demo.nahui.app`, §5b and §10 Step 13) is now itself retired. Real WhatsApp OTP means `demo.nahui.app` no longer offers a distinct experience from `nahui.app` — there is no separate demo deployment left to point a future campaign's button at. A future campaign built from this template should point its destination at `nahui.app` directly, and should account for the fact that arriving there now means completing a real WhatsApp OTP and creating a real, retained Business — a materially heavier ask than the disposable prototype this document's own reasoning (§1, §5b, §8) was built around.
...
**Companion documents:** `company/merchant-validation-campaign.md` — the organic campaign this plan follows. This document links to it rather than duplicating it. The questionnaire (Google Form, `https://forms.gle/ZZhtJEfee3viWY1h8`, Deliverable 3), the full incentive structure (Deliverable 4), and the four recommended screenshots (Deliverable 1c) all live there — read that document first if any of those specifics are needed; they are not restated in full here. **`company/merchant-validation-strategy-v2.md`** — the direct evolution of this campaign's approach, grounded in its real results (an onboarding-comprehension stall and a large-catalog vendor's entry-effort complaint, from the two genuine independent stranger responses this campaign produced), proposing hypothesis-specific video campaigns in place of the single generic one below.
...
| **"Prototype starts/completions" — not directly measurable by Meta** | Named honestly as a real gap | `demo.nahui.app` has no Meta Pixel — Meta itself still cannot report how many people who landed there actually completed a task, and that's unaffected by anything below. **Correction (updated this session, superseding this row's original "no analytics installed" framing):** client-side, count-only analytics do now exist independently of Meta — `@vercel/analytics` is mounted and firing, capturing automatic page views plus one custom event (`demo_questionnaire_cta_click`, live since the Reminder Banner build) with three more designed and pending a delivery-confirmation prerequisite (`company/merchant-validation-funnel-diagnosis.md` Stage 8; `product/02-ux/demo-mode.md`'s own analytics amendment, persisted in parallel with this correction). This does not mean the funnel is instrumented: no backend, Meta Pixel, or server-side event logging exists anywhere in this project (`company/merchant-validation-strategy-v2.md` §12), and even that one live custom event's own end-to-end delivery to the Vercel Analytics dashboard is itself unconfirmed as of this writing (same Stage 8 finding). Treated here as a real but narrow, client-side, count-only capability — not a resolved gap. |
...
**Hypothesis E (Product Owner's working reference: "H5") — Demo Mode reduces validation-environment cognitive load, freeing attention for evaluating the actual product.** The validation flow has been materially improved through a series of Demo Mode changes — a dedicated Demo Mode introduction, an explicit "no real SMS" disclosure, instructions to enter any 6-digit code, a persistent "Modo demo" indicator visible throughout the walkthrough, an easier way to restart the demo, and clearer communication that the questionnaire is part of the same experience, not a separate step (`product/02-ux/demo-mode.md`'s 2026-08-16/2026-08-18/2026-08-19 amendments — see `company/merchant-validation-strategy-v2.md` §0 for the confounded, pre-fix baseline this hypothesis is read against). **These changes are intended to improve the quality of the experiment itself, not to change the product.** The hypothesis: participants who use the current Demo Mode will spend less cognitive effort understanding the validation environment and more effort evaluating the actual product experience.
- *Supporting evidence:*
  - Fewer comments coding to SMS/OTP/verification-code/authentication confusion, or to believing the prototype is broken, among Section B's open-text answers (Q6, Q8, Q9) — read against the standard default threshold convention (`merchant-validation-decision-matrix.md`), inverted: supporting evidence is this pattern staying *below* the 3-of-10 recurrence threshold that would otherwise flag it as a live problem.
  - Higher prototype task-completion rate (Q5), compared directionally against the pre-Demo-Mode baseline (n=2 genuine respondents, `merchant-validation-strategy-v2.md` §0) — the same directional, non-statistical comparison Hypothesis C above already uses for the OTP-bug fix.
  - Higher click-through rate from the prototype into the questionnaire, now directly measurable via the `demo_questionnaire_cta_click` event fired from `product/02c-high-fidelity-prototype/src/screens/DemoMode/ReminderBanner.tsx` (instrumented this same session specifically for this question) — a real client-side event, not a hypothetical future measurement. No pre-existing baseline exists for this specific metric, since the event didn't exist before this instrumentation; it is reported as this round's own first-measured rate, read alongside the other three categories below, not as a standalone before/after comparison.
  - Fewer sessions abandoned before reaching the selling flow, read the same directional way as prototype completion above.
- *Invalidating evidence:*
  - Authentication-related confusion recurs at a similar frequency to before (meets or exceeds the standard 3-of-10 recurrence threshold).
  - No measurable improvement in prototype completion.
  - Participants continue to misunderstand the prototype as a production application (e.g., worrying about a real charge, a real customer seeing real data, or what happens to "their" account afterward).
- *Evidence threshold — supported vs. rejected, stated explicitly since H5 spans multiple evidence categories rather than one recurring free-text pattern:* A single-category trigger, as Hypotheses A-D above each use, doesn't fit cleanly here — a compound claim about *where a respondent's attention goes* isn't something any one metric can carry alone, the same reasoning `merchant-validation-decision-matrix.md` already applies when it cross-checks Q7 against Q5/Q6/Q8/Q9/Q11/Q12 before treating a workflow-fit pattern as more than a one-off. A composite read across categories is used, not a single bespoke number and not four independent, unrelated thresholds:
  - **Qualitative category** (auth/OTP-confusion comments): apply the project's own default threshold convention (`merchant-validation-decision-matrix.md`) directly, as an absence check — the pattern is treated as resolved if it recurs in fewer than 3 of a first batch of 10 new respondents, and treated as still live (invalidating) if it recurs in 3 or more.
  - **Quantitative categories** (completion rate, CTA click-through, pre-selling abandonment): each reported directionally against the thin existing baseline (n=2), the same caveat-carrying comparison Hypothesis C already uses above — no category alone is claimed as statistically confirmed at this sample size.
  - **Composite call:** H5 is provisionally read as **supported (candidate)** if the qualitative category stays below its 3-of-10 threshold *and* at least 2 of the 3 quantitative categories move in the improving direction. It is provisionally read as **rejected (candidate)** if the qualitative category meets or exceeds 3-of-10, *or* none of the three quantitative categories improves. Any other combination is logged as inconclusive/mixed, not forced into either label.
  - **Escalation from candidate to confirmed** follows the same convention as every other row in `merchant-validation-decision-matrix.md`'s default threshold convention: the same composite pattern holding at a second batch of 10 (n≥20 total), or at a proportionally consistent share as the sample grows, moves it from "candidate" toward an actual decision — never on a first batch of 10 alone.
  - **Precision note, matching `merchant-validation-decision-matrix.md`'s own correction:** the real worst-case 95% margin at n=10 is **±31pp**, not the looser ±20pp figure that only actually applies at n=20 — stated here explicitly so this newly-added hypothesis doesn't repeat the imprecision that document already had to correct twice.
  - **What n this becomes evaluable at, stated plainly:** **not evaluable at the current n=2 genuine-respondent count.** Both of those two respondents predate every Demo Mode amendment this hypothesis concerns, so they can serve only as H5's pre-fix baseline, never as an in-sample test of H5 itself. H5 needs at least **10** new, genuinely independent respondents who went through the current Demo Mode build (i.e., after the 2026-08-19 "Modo demo" label shipped, not merely after the earlier OTP or banner fixes) before even a first candidate-level read is possible, and **20** before that read can move toward confirmed.
- *Decision this feeds:* **A validation-process decision, not a Product Decision.** H5 is explicitly a hypothesis about the validation methodology itself — whether Demo Mode, as research-onboarding infrastructure, is doing its job — never about Nahui's product. Per this project's Decision Ownership taxonomy (`company/CLAUDE.md`), "Product Decision" specifically means a decision that changes product behavior, UX, or feature scope, routed through `product/02-ux/product-decisions.md` — H5's outcome must never be filed there, and must not, on its own, be used to justify any change to `company/backlog.md` or `product/02c-high-fidelity-prototype/BACKLOG.md`.
  - If supported: keep Demo Mode as a permanent part of the validation flow; continue refining it as the standard entry point for research participants.
  - If not supported: revisit the validation onboarding before concluding that product friction is the primary issue — i.e., don't let unresolved validation-environment confusion get misread as a real product-friction finding.

**Status: untested.** No batch of new, Demo-Mode-era respondents exists yet as of this hypothesis's addition (2026-08-19) — H5 is tracked forward, not assumed true, and must not be treated as either supported or rejected until the evaluable n stated above is actually reached.
...
9. **Learning agenda:** Five falsifiable hypotheses (reach, message, bug-confound, adoption-intent-by-channel, and Demo Mode's own validation-process effect — Hypothesis E/"H5," added 2026-08-19, untested), each with named supporting/invalidating evidence and the decision it feeds — a `knowledge-mentor` consultation on hypothesis structure is a recommended, not-yet-performed, follow-up.
