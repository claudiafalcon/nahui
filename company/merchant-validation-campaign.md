...
| Q7 | *(new, fifth pass; options revised sixth pass — M1, M2, N2)* Pensando en todo lo que acabas de hacer: si algo no te resultó como esperabas, ¿qué fue principalmente? | Multiple choice + otro | Usability-vs-workflow-fit-vs-expectation-mismatch classifier — a distinct signal from Q6, Q8, Q9, Q11, Q12, and Q15 — see rationale below |
...
| Q14 | Si cobrara por funciones extra, ¿preferirías cuota fija, porcentaje, o depende del precio? | Multiple choice + otro | H5 replication (`market-validation.md` H5); feeds a new `company/business-decisions.md` entry on pricing-*model type*, recommended by `marketing` for Main to create — **corrected sixth pass (Important finding): not Q11**, which is about billing-cycle *timing* for capability changes, an unrelated question the prior citation wrongly conflated with this one. See the Sixth-pass remediation record below for the recommended entry text. |
...
**Open item, logged rather than silently unaddressed (N3, sixth pass) — Q15's proximity to Section D's pricing questions.** The final-review pass flagged that Q15 (the forced single-change question) sits directly after Q13/Q14 (pricing), which may pull respondents toward naming a price-related "one thing" independent of what actually bothered them earlier in the walkthrough — an anchoring risk distinct from, and not resolved by, the placement rationale above (which addresses proximity to Q16/Q17's opt-in mechanism, not proximity to Section D's pricing content). This is judged to need stronger grounding than direct reasoning provides — genuine anchoring-effect methodology, not a settled convention the way N2's ordering fix was — so it is **not resolved unilaterally here**. Logged as an open item: **a `knowledge-mentor` consultation is requested** (question: does the observed anchoring risk from placing a forced single-priority question immediately after two pricing questions warrant restructuring Section D/E's order, and if so, what's the better structure — moving Section D after Q15, inserting a brief attention-reset transition before Q15, or another established mitigation?) — to be dispatched by Main, not by `marketing` directly, per the standing Consultation Pattern. Until that consultation returns, Q15's current placement stands, and any resulting recurrence pattern in the data should be read with this open risk in mind rather than treated as unambiguous signal.
...
Hypothesis-tier updates go into `company/market-validation.md` §6 (H1-H6, upgraded only with two independent sources, per that document's own bar). UX-specific findings route to `product/02-ux/product-decisions.md` or `product/02c-high-fidelity-prototype/BACKLOG.md`'s reprioritization. **Pricing findings (Q14) feed a new `company/business-decisions.md` entry on pricing-model type as evidence, not a resolution — corrected sixth pass: not Q11**, which is about billing-cycle timing, a different question the prior citation wrongly conflated with this one (see the Sixth-pass remediation record below for the recommended new-entry text). Anything that doesn't fit an existing open question gets classified (Architect/Product/Business Decision) and logged in the matching file — never resolved unilaterally. **The specific, per-question routing and numeric/pattern thresholds that trigger each of these actions are in `company/merchant-validation-decision-matrix.md`** — this paragraph states where things go in general; that file states exactly when.
...
- **N3 (Minor) — Q15 anchoring risk from proximity to Q13/Q14.** Not resolved unilaterally — logged as an explicit open item under Section E, with a specific `knowledge-mentor` consultation requested (for Main to dispatch) rather than left silently unaddressed or guessed at.

**Q7's copy has not yet been through `brand-guardian`.** Per the Product Owner's own instruction accompanying this remediation batch, that pass is deliberately deferred until after this round closes, so `brand-guardian` reviews stable, final wording rather than wording still in flux — not requested by `marketing` in this pass.
...
- **Q15's anchoring risk (N3, above)** — a `knowledge-mentor` consultation is requested, pending Main's dispatch, before this is considered closed.
- **The new `business-decisions.md` entry** recommended in the Sixth-pass remediation record above — pending Main's review and application; `merchant-validation-campaign.md` and `merchant-validation-decision-matrix.md` both already reference it as the corrected destination for Q14's evidence, so it should be created before this campaign actually runs and produces Q14 data with nowhere accurate to land.

**Next in the pipeline, per the Product Owner's own sequencing:** now that this remediation batch is closed and Q7's wording is stable, `brand-guardian` review of Q7's final copy, followed by `ux-critic`/`reviewer` re-verification of this whole batch.
