...
## 6. Final review round, 2026-08-19 — all three open questions closed

Three independent review passes ran against the open questions in §5, and a fourth, unrelated defect class (composite-seam artifacts) surfaced during the independent `ui-designer` review and has now been remediated. Status of each:

**(a) SB-4 — Hook screen choice (§5 item 2). Resolved: keep the scripted Selling-grid Hook.** Both `marketing` and an independent `ui-designer` review considered substituting the Idle hero screen (`home.md §3.4`) and agreed the scripted Selling-grid framing (`home.md §3.9`, as originally built) should stay. The Idle screen reads as a more polished standalone hero image, but the Selling-grid framing is what actually carries the Hook's loss-aversion premise ("$0 · 0 ventas," "Venta actual: (vacía)") — the Idle screen doesn't establish that tension on its own. No change made to Beat 1's screen choice. SB-4 above is retained as the record of the original reasoning; this entry closes the open question it flagged.

**(b) Montage density (§5 item 4). Resolved: the fast-cut concept itself is fine; the real problem was two composite-seam defects, now fixed.** The independent `ui-designer` review found the montage's jump-cut density reads as intentional ("fast-forwarded," not "choppy") once watched at full speed — no change needed to shot count, hold durations, or cut pacing. What that same review actually flagged as a genuine defect was pixel-level: two composited frames (`frames/m3-sellinggroups.png`, `frames/m6-registrar.png`) had seams that cut through live content — a stray dashed-line/red-curve fragment above the "Continuar" button in `m3-sellinggroups.png`, and clipped mid-character text ("+ Agregar otro producto," "Descartar") in `m6-registrar.png`. Both were recropped so the seam falls in genuinely empty background space in both source screenshots. A full audit of every other composited frame in the sequence (per the same review's instruction, not assumed limited to the two reported) found three further instances of the same defect class that had gone unreported: a clipped glyph descender in `frames/m2-identity.png` (bottom of "La guardamos con tu negocio…," cut by ~3-4px), a mid-element cut through the "Activar plan de pago" button in `frames/m1-welcome.png`'s single top-crop, and a cut through the live product tiles ("Playeras"/"Blusas") in `frames/04-selling.png`. All five frames were recropped using the same rule (seam only in confirmed-uniform background pixels, verified per-frame rather than by eye), the six affected shot MP4s were regenerated at their original durations, and the final video was re-concatenated. Verified by extracting frames from the re-concatenated output at each affected shot's timestamp and confirming the fix landed in the actual assembled video, not just the source PNGs.

**(c) On-screen "stills, not a recording" disclosure (§5 item 1). Resolved: `brand-guardian` returned a Major finding requiring disclosure; spec applied as directed.** Copy (verbatim): "Capturas reales del prototipo — no es una grabación." Typography: Inter Regular (not Fredoka), small-body tier of the type scale (`brand-guide.md`'s type scale), rendered in a muted warm gray subordinate to the Hook's bold Fredoka question. Placement: top-centered in the existing empty gray band above the phone-screenshot crop in `frames/00-hook.png` (the band was already visually empty at full width — no existing content displaced). Timing: silent, present for the full Beat 1 hold (0:00–4.3s), hard-cut away with the rest of Beat 1 — no separate segment or duration change needed since it's baked into the existing Beat 1 frame/shot. Beat 1's segment MP4 (`01-hook.mp4`) was regenerated with the updated frame and original audio/duration preserved, and the final video re-concatenated.

**Overall status: all three open review questions from §5 are closed**, plus the seam-defect class found during the independent review (both originally-reported instances and three additional siblings found via full-sequence audit) is remediated. Total runtime is unchanged (29.39s — §3's item 3 remains open only in the sense that no further trim was made in this pass; it was not in scope for this round). Final asset: `company/campaign-assets/campaign-b-draft/final/campaign-b-draft.mp4`.
