...
## Secondary strategic objective: Nahui as an AI-engineering laboratory (Product Owner directive, 2026-08-17)

Regardless of commercial outcome, Nahui should also become a real-world platform for learning modern AI engineering practices — agentic AI, long-term memory architectures, multi-agent collaboration, RAG, knowledge management, semantic search, tool calling, AI orchestration, workflow automation, evaluation frameworks, AI observability, human-in-the-loop patterns, multimodal AI, reasoning models, MCP, and AI security/governance. **This is not a mandate to force AI into features where it provides no customer value** — it's a tiebreaker: when two architecturally comparable implementations (genuinely similar cost/complexity, both already justified by a real need) are on the table, prefer the one with greater long-term educational value.

The operational rule — including its explicit scope boundary (it governs backend/infrastructure/tooling decisions, never merchant-facing UX/UI, never the Product Backlog Ownership priority order below, never grounds to reopen an Approved Low-Fidelity spec) — lives in `product/00-foundation/architecture-principles.md` principle 8, since that's what `architect`/`ui-designer`/`reviewer` actually check when evaluating a technical trade-off. See `product/00-foundation/decision-log.md` D49 for the full reasoning, including three open Business Decision flags (schedule/cost latitude against `backlog.md` #1's unmet bar, whether this extends to backlog *sequencing* — it doesn't, as scoped — and whether "comparable cost" excludes new recurring paid services) not yet resolved by the Product Owner.

**Every decision made under this lens, and the lesson learned once it's built, gets recorded in `company/ai-lab-decisions.md`** — the project is meant to be both a real product and a long-term AI-engineering lab notebook, not just the former with a slogan attached.
...
- High-Fidelity UI work (React/TypeScript, in `product/02c-high-fidelity-prototype/`, strictly on top of an Approved Low-Fidelity spec and an Approved architecture review — the standing execution medium as of D42, and as of 2026-08-30 the *only* one — Figma is fully removed, not merely optional) → `ui-designer`.
...
3. Dispatches the next work item(s) immediately — applying the `backlog-prioritization` skill first only when the next step genuinely isn't obvious from backlog/decision-log state alone, not as a standing checkpoint before every dispatch.
...
#### Architect stays productive, not just on standby; backlog refinement is Main's own standing work

`architect` is not a purely reactive role that only activates when an ambiguity happens to surface. When active delivery (UX/UI builds, review cycles) is progressing smoothly and it has no open question to resolve, Main proactively assigns it forward-looking work that prepares upcoming phases — without touching or blocking active delivery: architecture readiness review for the next phase (are aggregate boundaries, the domain model, and open decision-log items actually implementation-ready), identifying gaps that would block `builder` before they're discovered mid-build.

This is a genuine, read-only research/planning deliverable (the agent stays read-only, no code or doc changes) that gets reported back and, if it surfaces something actionable, routed through the normal Decision Ownership process — not busywork, and not a reason to interrupt active delivery to make room for it. If there's truly nothing forward-looking to prepare (rare), it stays idle rather than manufacturing a task.

Backlog refinement, sequencing/roadmap prep for the next phase (e.g., `product/03-build` once Medium-Fidelity closes), release planning, and risk analysis on what's coming next are no longer a separate agent's forward-looking assignment — they're Main's own standing work, applying the `backlog-prioritization` skill directly whenever delivery is quiet enough to make room for it (see `company/bitacora.md`, 2026-08-12, for the retirement record of the former `planner` agent).
...
**Explicit exit criteria (added 2026-08-12, closing the gap that let the `184:1503` reconvergence hunt run 5+ open-ended rounds).** The rule above says cycles should converge; these four points are what actually enforces it:

1. **Distinguish core-loop severity from secondary-path severity, explicitly, at the start of any fix dispatch.** A Blocker on the actual validated-hypothesis path (registration → sell → receipt) always gets fixed, full stop — no round cap applies there. A Blocker on a secondary path (event-detail edge states, quick-session shortcuts, settings corner cases) gets **one** fix-and-verify round; if a *sibling* instance of the same defect class surfaces afterward, log it under Known Limitations (below) rather than automatically opening another full round — unless an exhaustive caller-audit (point 3) is explicitly chosen instead.
2. **Cap reactive, tester-discovery-driven rounds at 3.** After the third round of `merchant-user-tester` surfacing new instances of the same defect class, Main stops and produces a checkpoint for the Product Owner rather than auto-continuing to a 4th round.
3. **An exhaustive caller-audit is the one sanctioned exception allowed to run to full closure**, because it's designed to *end* the pattern (enumerate every caller of every known-affected shared component once, classify each, fix every real instance in one pass) rather than extend it. Reach for this explicitly once round 2–3 keeps finding new siblings of the same defect class — it's a different move from another reactive tester walk, not just "one more round."
4. **Known Limitations, maintained continuously, not just at the end.** Anything accepted-not-fixed per points 1–2 gets logged in `product/02-ux/ux-critic-findings.md`'s "Known Limitations" section the moment it's accepted, not batched into a final report — same non-deletion discipline as every other findings log in this project.

The objective of any remediation cycle is not zero findings at any cost — it's sufficient validated quality to move the product forward.
...
**Merchant and customer journeys are separately scoped, tested separately (2026-08-10, Product Owner standing rule).** `merchant-user-tester`/Ana tests the Merchant Application only. Her journey ends the moment a merchant-side task completes — for the Digital Receipt specifically, that means confirming the correct content (including whether a Claim Token QR is visibly present) is showing, never tapping, scanning, or following that QR into the customer-facing claim/registration flow. `product/02-ux-loyalty/` (D38) is a structurally separate deploy target with its own persona/tester requirements, not yet defined — a QR-destination or customer-flow finding surfaced incidentally during a merchant walk is recorded (`company/active-artifacts.md` or that folder's own tracking) as a customer-flow concern for a future, separately-scoped test, never treated as a blocker on the merchant journey it was found in. Main does not ask the Product Owner to take a customer-facing-journey action (e.g., generating a public share link) as part of closing out a merchant acceptance test — that request belongs with whatever workstream actually takes up the customer-facing journey.
...
**Why this shape, specifically:** it keeps three distinct responsibilities from ever blurring into each other — the specialist's domain judgment, Main's orchestration, and the consulted agent's evidence-gathering — the same separation-of-concerns discipline this project already applies throughout the rest of the pipeline (`ux-critic` vs. `reviewer`, `architect` vs. sequencing). A consulted agent that started making decisions would collapse into a reviewer or a second specialist; a Main that started deciding when consultations are needed would collapse into the specialist it's supposed to be dispatching, not supervising.
