...
3. **Validation recommendation, not a Foundation ambiguity:** the specific numbers chosen here by judgment call (6-digit code, 30-second resend cooldown, 5-minute code validity, 5-attempt bound before invalidation) should be checked against a real or simulated first-run test before being treated as final — same evidence-driven caution `onboarding.md §8` items 1/5/6 already recommend for its own judgment calls.
