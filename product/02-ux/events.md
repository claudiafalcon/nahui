...
- **"Disponible en general" = Business-wide total minus whatever's allocated to every *other* open `EventAllocation`** — deliberately *includes* what's already allocated to *this* Event, since that stock is already hers to freely reassign within this screen, not "elsewhere." **[Corrected 2026-09-20, `decision-log.md` D82 — the stepper's ceiling is no longer this figure.]**
  This bullet previously said the two were exactly equal, "no separate, unexplained cap." That was
  wrong in a way that produced a wrong number under a success message: "Disponible en general" counts
  **all** available units, tagged and untagged, while the manual commit pool is **untagged-only**,
  enforced server-side in `_fifo_commit_to_allocation`, which is deliberately partial-fulfillment
  tolerant. On a Product with 8 available units of which 5 carry tags, she could type 8, tap Guardar,
  see a success confirmation, and have 3 units committed.

  **The manual stepper's ceiling is now derived from the pool the manual write can actually consume:**
  this Product's `available` **untagged** units, plus this allocation's own outstanding
  `fifo_assignment`-source units. **No NFC-eligibility predicate is applied** — eligibility governs
  future tagging work (D71/D73), and a merely-untagged unit is manually committable regardless of
  whether its Product ever opted into tagging.

  **"Disponible en general" itself is unchanged and still displayed**, because it remains the honest
  answer to its own question — how many of this Product are available to this Event across the
  business. The two are simply different derivations of different questions, and the cap is explained
  rather than unexplained: where they differ, the difference is this Product's tagged units, which
  belong to the scan pool. **The fuller answer is §3.21's own "sin tag · con tag" split**, which shows
  both pools separately instead of only capping one; that remains specified and unbuilt, and D82
  deliberately does not pre-empt it.
- **The "sin tag · con tag" split only renders when `Business.nfcPerProductEnabled = true` *and* this specific Product has ≥1 unit with `tagId != null AND status = 'available'`**
- **Two different displays, two different render conditions — split apart 2026-09-21 (`decision-log.md` D84), because one condition was doing both jobs and got one of them wrong.**
  - **(a) The "sin tag · con tag" *availability* decomposition** — the bullet directly above. A question about the **scan candidate pool**, so its condition is correctly about tagged units still `available`.
  - **(b) The committed-scan disclosure on the expanded row ("N escaneadas") and the collapsed summary's scan component** — a question about **this allocation's already-committed units**, so it renders on `quantityRemainingBySource(allocation, 'scan') > 0`. **It must not inherit (a)'s condition**: a row whose tagged units have *all* been scanned into this allocation has zero tagged-`available` units left, so (a)'s condition would hide the disclosure at exactly the moment the merchant most needs it — she has committed garments by scan and nothing on screen would say so.
  - Not a reopening of `decision-log.md` D81, which ruled on the *gate*. The same "same sourcing rule, question-appropriate scope" distinction D80-vs-D81 already drew, applied one level further.
  - **The collapsed summary stays the combined live total** (§3.21's own requirement), derived as staged manual quantity + `quantityRemainingBySource(allocation, 'scan')`. That simplification is justified *by* the expanded breakdown existing — which is why (b) is part of the correctness fix, not optional polish: without it the summary and the stepper show different numbers with nothing on screen explaining the gap. *(capability clause restated 2026-09-20, `decision-log.md` D81 — "this Business has NFC capability" was pre-D79 wording from when `Session.operatingMode`/NFC Readiness still existed; the live Business-level gate is `nfcPerProductEnabled`. **The tagged-unit half was already correct and is deliberately unchanged** — it is the same live-unit-state derivation D81 applies to §3.22a's list-level gate, and this row-level rule already had it right. Note this row-level path is specified here but **not yet built**; `MercanciaParaEsteEvento.tsx` records §3.22 and this split as out of scope, which is why §3.22a's gate was the only one in the shipped prototype.)* — otherwise a single plain "Disponible en general: N" figure, identical to `inventory.md` §3.4's own Catalog-row language, and the row shows only the manual stepper (no scan affordance at all). Reuses the exact "tienen tag" plain-language register `home.md` §3.6a already established for surfacing NFC state to Ana — never "NFCTag," "reserved," or "allocatedUnitIds."
...
- **Manual quantity stepper — reuses `inventory.md` §3.6's Cantidad stepper shape (`[−]`/`[+]`/typed entry via `teclado numérico`) with one deliberate difference: floor is 0, not 1.** Cantidad's floor-of-1 exists because "0 units received" isn't a real receiving event; here, "0 units allocated to this Event" *is* a real, valid decision (she may simply choose not to bring a Product at all) — the two fields share a shape, not an identical business meaning. **The value and the ceiling are both on the manual (`fifo_assignment`) basis — named together, deliberately (`decision-log.md` D84, 2026-09-21).** D82 amended this bullet to define the *ceiling's* basis and said nothing about the *value's*; that silence is the seam a silent over-commit lived in for one pass, because the value was initialised from the combined `quantityRemaining` while `save_event_allocations` reads the submitted number as the `fifo_assignment` target. **Value = this allocation's outstanding `fifo_assignment` units** (never the combined figure, and a successful scan never increments it — a scan is already committed server-side and is not a manual stage). Ceiling = **the manual pool**, not the row's "Disponible en general" figure **(corrected 2026-09-21, `decision-log.md` D82 — this bullet previously said the two were the same, and the correction landed on the "Disponible en general" bullet above without being carried through here, leaving the two bullets contradicting each other for one pass)**: available **untagged** units plus this allocation's outstanding `fifo_assignment` units. Where it differs from "Disponible en general," the difference is this Product's tagged units, which belong to the scan pool; `[+]` goes inert at the ceiling (symmetric to Cantidad's own inert-at-floor `[−]` behavior), and typing past it clamps to the ceiling with a one-line inline message ("Solo tienes N disponibles.") rather than a rejected/blocked keystroke.
...
- **"Guardar cambios" commits only the manual (staged) quantities, and the number it sends is the `fifo_assignment` *target*, not a delta and not a combined total** (`decision-log.md` D84). The server computes the signed delta against this allocation's own live-remaining `fifo_assignment`-sourced count — a contract stated verbatim in `save_event_allocations`' own header since it was written. Sending a combined figure against that baseline inflates the delta by the scan-committed count and silently over-commits untagged units. Leaving this screen before tapping it discards only the unsaved manual edits (already-scanned units stay committed) — a deliberate choice for internal consistency with this document's own precedent (§10: "Nuevo Evento's draft is not auto-preserved across interruption"), not imported from Inventario's different draft-preservation posture.
...
- **Resolved 2026-09-15 (`decision-log.md` D69, `product-decisions.md` Q29, Product Owner live-testing report) — rows now resolve `User.displayName` first, phone only when it isn't set.** The gap this bullet used to name ("No display-name field exists on `User` yet") is closed — `settings.md` §2.7's own corrected three-tier resolution order (`User.displayName` → phone → role-only fallback) is reused here verbatim rather than reinvented, narrowed to its first two tiers only: every row on this screen is, by construction, an *active* `BusinessMembership`, so the third tier ("Alguien de tu equipo," used elsewhere for a Membership with no phone and no name) never applies here — a phone-verified or otherwise-identified active SELLER always has at least the phone tier to fall back to, the identical reasoning that already made phone this screen's own unconditional baseline before D69.
...
- "Disponible en general" (§3.21) is computed automatically as the
  ...
  **The manual stepper's own ceiling is computed separately and just as
  automatically** (`decision-log.md` D82): available untagged units plus
  this allocation's outstanding `fifo_assignment` units — the pool the
  manual write can actually consume. She is never asked to work out which
  of her units are taggable, and never allowed to type a number the write
  cannot honour
  across Events.
...
- **[Resolved 2026-09-20 — `decision-log.md` D82, fixed in its own pass immediately after D81, not bundled with it.]** The manual stepper's ceiling was not honest on a Product with residual tagged units. `disponibleEnGeneral()` counts **all** available units, tagged and untagged, and §3.21 makes it the manual stepper's ceiling — but the manual commit pool is untagged-only by server-side predicate, and `_fifo_commit_to_allocation` is deliberately partial-fulfillment tolerant ("fewer candidates than requested is not an error... never raises"). So she can type 8 against an honest-looking ceiling of 8, tap Guardar, receive a success confirmation, and have 5 units committed. §3.21's own "sin tag · con tag" split exists precisely to keep that ceiling honest, and it is **unbuilt**. **More merchant-visible than the defect D81 fixed** — a wrong number under a success message, rather than a missing button. Fixing the gate does not close it. **Fixed:** the ceiling now derives from available untagged units plus this allocation's outstanding `fifo_assignment` units (D82). §3.21's "sin tag · con tag" split — the fuller answer, showing both pools rather than capping one — remains specified and unbuilt, and D82 deliberately does not pre-empt it.
