...
scheduled → cancelled MANUAL. The only merchant-initiated transition in this
                      ...
                      (§3.11). See §10 for why "cancel" isn't offered once an
                      Event is active.
...
- "Día 2 de 3" — the "de 3" is unambiguous (Event's own scheduled span,
  `endDate − startDate + 1`, plain calendar arithmetic on data she typed in
  §3.6). "Día 2" is the part Q1 leaves open — see §8. Denominator and
...
- Pasados cards show a thin, ambient one-line summary (ventas, $ total, días)
  ...
  6 ventas," `home.md` §3.7), not a breakdown. Tapping through to §3.15 hands
  off to Resultados for anything beyond this one line, per Q7's resolution
  (`architect-questions.md`).
- **Pasados also renders a zero-Session Event gracefully** — "Feria Ixtapan ·
  Sin ventas registradas" — instead of mechanically applying the standard
  card shape, which would otherwise read as "0 días · 0 ventas · $0." This is
  the list-level counterpart to §3.16's detail-screen treatment of the exact
...
- A zero-Session Pasado renders identically here — same card shape and copy
  as §3.4's "Feria Ixtapan" example (see §3.4's annotation for the full
...
│  [ ______________________ ]      │
│                                │
│  [      Guardar evento       ]   │  disabled until Nombre + Tipo + Empieza
├───────────────────────────────┤
│ Hoy  Inventario [Eventos] Resultados │
└───────────────────────────────┘
```
...
- Nombre is freeform, her own naming, same as it's already referenced
  throughout `home.md` (e.g., "Bazar Plaza Norte") — never a system-generated
  label.
- Lugar is optional plain text, no map/geocoding — no such capability is
  described anywhere in the Foundation, and inventing one here would be
  exactly the kind of unvalidated feature `vision.md`/`CLAUDE.md` warn against
  building ahead of need.
- Only Nombre, Tipo, Empieza are required; Termina is always populated
  (auto), Lugar is optional. No Supplier/cost-style hidden fields apply here
  — Eventos has nothing analogous to Inventario's deliberate-exception fields.

### 3.7 Elegir tipo — picker sheet
```
┌───────────────────────────────┐
...
- **Whether this list is genuinely closed or open-ended is not actually
  ...
  (`inventory.md` §3.8), which explicitly supports adding a new value. This
  spec conservatively shows no "add a new type" affordance pending that
  answer, rather than asserting either way. Escalated to Architect and logged
  as Q6 in `product/02-ux/product-decisions.md` (reclassified from
...
### 3.8 Guardar evento — saving / error
```
┌───────────────────────────────┐   ┌───────────────────────────────┐
│                                │   │        Guardando…              │
│        ▢▢▢▢▢▢▢▢▢▢▢▢            │   │                                │
...
- Identical convention and identical failure guarantee as `home.md`
  ...
  already typed. *global-principles.md*, "the best interface stays out of
  the merchant's way."

### 3.9 Post-save confirmation (ambient, returns to Events list)
```
┌───────────────────────────────┐
│  Eventos                       │
...
### 3.10 Event detail — scheduled (not yet active)
```
┌───────────────────────────────┐
│ ← Eventos                        │
│  Expo Toluca                     │
...
└───────────────────────────────┘
```
- Passive info (name, type, dates, place) + exactly one action. No edit
...
### 3.11 Cancelar evento — confirmation
```
┌───────────────────────────────┐
...
### 3.12 Post-cancel confirmation (ambient, returns to Events list)
```
┌───────────────────────────────┐
│  Eventos                       │
...
└───────────────────────────────┘
```
- The cancelled Event simply no longer appears anywhere en this list (not
  moved to Pasados — it never happened). No "Cancelados" archive is designed
  — see §10, §11.

### 3.13 Event detail — active, no Session opened today
```
┌───────────────────────────────┐
│ ← Eventos                        │
│  Bazar Plaza Norte                │
...
### 3.14 Event detail — active, Session already open today elsewhere
```
┌───────────────────────────────┐
│ ← Eventos                        │
│  Bazar Plaza Norte                │
...
- **Corrected per Architect's resolution of Q7** (`product/02-ux/architect-questions.md`,
  ...
  elsewhere. Corrected to passive identity (name, type, dates, place) + a
...
- **Remediation (EVT-M3): the one-line ambient summary is carried through
  ...
  elsewhere in this doc (Pasados list card's one-line summary, §3.4; §3.13's
...
### 3.16 Event detail — closed/past, zero Sessions (never attended)
```
┌───────────────────────────────┐
│ ← Eventos                        │
│  Bazar Metepec                    │
...
### 3.17 Defensive fallback / load error
```
┌───────────────────────────────┐
...
Open Eventos tab
  → resolve (§2, automatic)
      → load fails ──────────────────────→ fallback (3.17), Reintentar
      → no Event ever scheduled ─────────→ cold start (3.3) → Agendar evento
      → Events exist ─────────────────────→ Events list (3.4/3.5)

Events list:
  tap "Agendar evento" → Nuevo Evento (3.6)
  tap a Próximo card  → scheduled detail (3.10)
      → [Cancelar evento] → confirm (3.11) → Sí, cancelarlo →
        ambient "Evento cancelado" (3.12) → list, card removed entirely
  tap an Activo card  → active detail (3.13 or 3.14, depending on whether a
  ...
  tap a Pasado card   → closed detail (3.15, or 3.16 if zero Sessions) —
        3.15 shows the same one-line ambient summary already seen on the
        list before handing off
        → [Ver resumen en Resultados] → leaves Eventos for Resultados

Nuevo Evento (3.6):
  fill Nombre + Tipo (→ picker 3.7) + Empieza (Termina auto-fills) + Lugar (opt.)
  → tap "Guardar evento"
      → saving (3.8) → error → Reintentar → saving again
      → success → Events list, ambient "Evento agendado ✓" (3.9), card placed
        in Activo o Próximos purely by date — never a manual choice
...
Elsewhere:
  Home's upcoming-Event card (home.md §3.5) → tap → scheduled detail (3.10)
...
7. Elegir tipo — picker sheet
8. Guardar evento — saving (near-instant/slow) and error
9. Post-save confirmation (ambient)
10. Event detail — scheduled (not yet active)
11. Cancelar evento — confirmation
12. Post-cancel confirmation (ambient)
13. Event detail — active, no Session opened today
14. Event detail — active, Session already open today ("Vendiendo ahora")
15. Event detail — closed/past (identity + one-line ambient summary +
    hand-off to Resultados)
16. Event detail — closed/past, zero Sessions (never attended)
17. Defensive fallback / load error

## 6. Minimum step count

| Scenario | Taps / entries | Why it can't be fewer |
|---|---|---|
| Agendar un evento nuevo, un solo día, sin lugar | 1 (Agendar evento) + 1 typed Nombre + 2 (Elegir tipo, seleccionar) + 1 (Empieza) + 1 (Guardar) = 5 actions | Nombre, Tipo, and a start date are the minimum facts needed to make Home's future resolution (`home.md` §2) work at all — not padding. Termina and Lugar add zero extra steps in the common case. |
| Agendar un evento de varios días | Same as above + 1 (editar Termina) | The one unavoidable extra step for the minority multi-day case; still cheaper than asking "¿cuántos días?" up front for every Event. |
...
| Ver el resumen completo de un Evento pasado | 2 (tap card → Ver resumen en Resultados) | Eventos identifies the Event and shows its one-line ambient summary in 1 tap (§3.15); the full day-by-day breakdown lives in Resultados per Q7's resolution, one hop further. |
...
- Rollup totals (días, ventas, $ total) — computed automatically from
  ...
  including its "Sin ventas registradas" case) and its echo on §3.15 — one
...
- Cancelled Events disappearing from the list entirely, with no separate
  "archive" step required to hide them (§3.12).
...
- **Q1 ("Día N" counting) — not new.** Logged in
  ...
  summary and its §3.15 echo post-EVT-M3; §3.13/§3.14's single ambient row)
  ...
  designed. The thin one-line summary restored to §3.15 by EVT-M3 is the
...
- **Q6 — Is the Event `type` field a closed enum or an open, extensible
  ...
  whether the list is closed or merchant-extensible). §3.7's picker
...
undefined state the way Q1/Q2/Q3 do.
...
- *"The fastest interaction is the one that never happens"* — Termina
  ...
  ambient, not screens requiring a dismiss tap (§3.9/§3.12); empty list
...
- *"Technology should disappear"* — loading states stay silent unless
  genuinely slow (§3.1/§3.2/§3.8), identical convention to the other two
...
- *"Selling is a state, not a navigation destination"* — the active-Event
  ...
  off to Hoy (§3.13/§3.14).
- *"Business language before technical language"* — copy uses "evento,"
  "agendar," "evento," "vendiendo ahora" — never "Event," "Session,"
  ...
  oficina, §3.7) — no leftover English domain term, per EVT-M1's fix.
...
- *"The best interface stays out of the merchant's way"* — a failed
  Guardar evento never drops her typed data (§3.8); a closed Event with zero
  Sessions is shown factually, not as a failure state, both in its detail
  screen (§3.16) and in its list card (§3.4/§3.5, EVT-M2 remediation).
...
- *#4 (internal-only entities never leak into user-facing language)* — the
  ...
  only "Día N," matching Home's own established convention.
- *#6 (one-way dependency direction)* — Eventos only reads Selling data
  (Sessions/Sales aggregated by `eventId`); it never writes into Inventory,
  and "Ver resumen en Resultados" (§3.15) is a hand-off, not a duplication of
  Resultados' own analytics — confirmed by Architect's resolution of Q7, not
  just an aspirational framing. The one-line ambient summary restored to
  §3.15 by EVT-M3 is the same reused fact already computed for the list card
...
- **Event status transitions are automatic/date-driven except cancellation**,
  ...
  (§2, §3.11). Chosen because it needs no merchant upkeep ("never ask
...
- **Eventos does not own the per-Event summary/breakdown — corrected per Q7's
  resolution.** The original draft put a full days/totals rollup in §3.15;
  ...
  status/navigation ("drills into their Sessions"), not analytics. §3.15 now
...
- **Event type "Market" naturalized to "Tianguis" in the Tipo picker (§3.7)**
...
- **Pasados list cards render a zero-Session Event gracefully (§3.4/§3.5)**
  ...
  before its detail view (§3.16) already handles the same case gracefully.
- **§3.15 carries the same one-line ambient summary already shown on the
  ...
  role; §3.15 had dropped this entirely, leaving §1's stated Merchant Goal
...
- **Nuevo Evento's draft is not auto-preserved across interruption**, unlike
  ...
  form where the cost of re-typing (one name, one date pick) is low enough
  that neither a silent-preserve mechanism nor a Descartar confirmation is
  justified.
...
- Editing an already-scheduled Event (fixing a typo in Nombre, adjusting
  dates before it goes active) — not designed; today's flow only lets her
  view or cancel (§3.10). A real gap if real usage shows frequent
  re-scheduling.
...
- A richer Lugar field (map pin, saved locations she reuses) — plain text is
  sufficient for v1; no geo capability exists in the Foundation to build on
  yet.
- A reminder/notification ahead of an upcoming Event ("mañana empieza Bazar
