...
- "Guardar precio" writes the new value directly to `Product.defaultPrice`
  ...
  her typed value intact. **Correction (`reviewer` finding, 2026-09-06, on the built code):** this passage previously claimed the write "carries the same stable idempotency-key guarantee every other retryable write in this doc family already carries," per `decision-log.md` D30/`architecture-principles.md` #7. That's not accurate against the actual implementation — no idempotency key is generated for this write. **Superseded 2026-09-21 (`reviewer` I-4): this is no longer true.** `editPrice` now holds a `crypto.randomUUID()` key in a per-attempt ref, generated once, reused across retries, cleared on success — `BACKLOG.md` §F records the family closed 2026-09-13. The 2026-09-06 correction is retained as the record of a real defect that has since been fixed, not as a current statement. Same pre-existing gap as `commitLot()` (`product/02c-high-fidelity-prototype/BACKLOG.md` §F), now confirmed to also cover `editPrice`/`setProductPhoto`, not fixed here.
...
### 3.15 Asignar tags — error, tag already assigned (trigger condition narrowed 2026-09-21, `decision-log.md` D83 / RFC 0018)

**The Spanish copy below, this state's layout, and every interaction in it are unchanged. Only the condition that reaches it narrows.** Previously this fired whenever the scanned identifier already had *any* `nfc_tags` row — including one belonging to a unit sold months ago, which is what made legitimate reuse of a returned physical tag impossible. It now fires **only when the tag's one open attachment belongs to a unit that is still on hand** (`available` or `reserved`) — a genuine conflict, two garments claiming one physical tag. When the open attachment belongs to a `sold` or `removed` unit, the scan **succeeds silently**: the prior window closes and a new one opens in the same transaction (D83). She is told nothing, asked nothing, and sees no new state — to re-attach a tag she must be holding it, and holding it is the proof it came back. A spec-accuracy amendment, not a design change; no new screen, state, or copy is introduced.
...
- **Near-instant:** the label quiets and un-quiets silently, landing on the new value. The word and the control are at full strength throughout, including in this brief window. **The durable reason is not reading speed: the control's attempted position is not a transient message at all.** It is displayed on tap and *persists* — on success it simply stays there, and it only ever moves back on failure. Nothing flashes, which is why suppressing the message and requiring the control are not in tension. No message.
...
- **`[ Guardar nombre ]` writes `Product.name` directly and closes back to §3.19**, updated — its heading, its Nombre row, and its marker (when no photo is set) all now reading the new name. `[ Cancelar ]` discards and returns unchanged. Follows the same near-instant/slow/error save convention as every other write in this document (§3.10/§3.11); a failed save leaves the sheet open with her typed value intact. Exposed to a client-initiated retry, so *architecture-principles.md* #7's stable-idempotency-key requirement applies — stated as a requirement on the build, against the same standing `BACKLOG.md` §F gap named at §3. **Requirement met as of this build (corrected 2026-09-21, `reviewer` I-4):** each of these writes holds a `crypto.randomUUID()` key in a per-attempt ref, generated once, reused unchanged across retries of that attempt, and cleared on success. `BACKLOG.md` §F's own entry records this family closed 2026-09-13, together with the `reviewer` Blocker fix that established the pattern — so the back-reference below is retained as provenance, not as an open gap. The requirement sentence stays because it is a requirement on any future write of this class, not because this one is unmet.4a/§3.4b and §3.19's NFC switch.
  ...
  Level 2:
    ...
    tap the NFC row — LIVE SWITCH ONLY (shape 3; gated nfcPerProductEnabled
      ...
      Product.nfcTaggingEnabled directly, in place
        → label quiets, word+control immediately show the attempted
            value (neither is ever inside the dim, §3.9); further taps
        ...
        → slow (>~1.5s) → trailing value reads "Guardando…", control holds
            the attempted position, ONLY the already-read label quiets
            (settings.md §3.9 — the attenuation reaches neither the
            status word nor the control)
...
7a. **[Resolved 2026-09-20 — `decision-log.md` D81. Text retained per this document's non-deletion discipline, same treatment as items 5 and 7.]** The defect named below is fixed: `events.md`'s allocation entry point no longer reads `Product.nfcTaggingEnabled` at all, and is re-derived from `Business.nfcPerProductEnabled` + ≥1 unit with `tagId != null AND status = 'available'`. **Wider than described below**: the gate was a Catalog-wide existential, so when the last flag went false it stranded tagged units of *every* Product, business-wide — and barcoding was only one of two ordinary routes there, D71's off-toggle being the other. **No RPC changed** — the write paths were already pure unit-state. Original text follows.
...
- **The NFC row's pending, failure, and revert-on-failure behaviour are fully specified (closes `ux-critic` M1).** It is the only control on §3.19 that saves instantly and had the same silent-failure gap already flagged elsewhere. Optimistic display on tap, only the already-read label quieted (`settings.md` §3.9 — never the status word, never the control), silent when near-instant, `Guardando…` when slow, **revert to the last stored value on failure** plus one inline sentence scoped to that row, the row itself as the retry, and a defined interruption behaviour (re-read from the server at page load, never a persisted `Guardando…`, never a silently replayed attempt). The optimistic display is only safe *because* the revert is guaranteed; the two are one decision.
