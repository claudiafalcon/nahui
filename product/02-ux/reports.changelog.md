...
production-testing defect): CSV → real `.xlsx`.** The React build (the
same build this document's own `ux-critic`/`reviewer` code-review rounds
had just found clean) shipped with a plain-text CSV, UTF-8 BOM included.
Tested on a real device, every accented character rendered as mojibake
in the Google Sheets Android app ("Sesión" → "SesiÃ³n," etc.) — the
signature of correct UTF-8 bytes read back as Latin-1. Verified directly
that the app's own generated bytes were themselves correctly encoded;
the defect was the receiving mobile app's own CSV encoding
auto-detection, which a BOM doesn't universally guarantee against.
Corrected to generate a genuine `.xlsx` Excel workbook (SheetJS `xlsx`,
dynamically imported so the ~450KB gzipped library only loads the first
time a merchant actually taps "Descargar Excel," not on every app load)
— OOXML's own format has no equivalent encoding ambiguity, verified via
a build-then-reread round-trip test. `reports.md` §2/§3.19/§3.20/§4/§7/§9/§10
corrected to describe "Excel (.xlsx)" in place of "CSV" throughout; row
shape, columns, and every other design decision unchanged. Code-side:
`salesExportCsv.ts` renamed `salesExportFile.ts`,
`buildSalesExportCsv`/`triggerCsvDownload` replaced with
`buildSalesExportWorkbook`/`triggerXlsxDownload`, `ExportarVentas.tsx`
updated to match (button now reads "Descargar Excel," filename extension
`.xlsx`). `npm run build` clean, independently re-confirmed by Main.
