...
**As of `product/02-ux/acceso-dm.md`, `path = 'demo'` is intentionally shared by two distinct real-world entry routes, not one** (`reviewer` finding, 2026-09-03) — the self-serve "Ver un ejemplo" path documented in this section, and a separate, Product-Owner-issued "Acceso DM" link that composes the same `completeOnboarding('demo')` write (plus the same identity/seed calls) from outside this document's own flow entirely, before Authentication ever renders. Both produce a domain-identical Business — same capability row, same seed — by deliberate design (`acceso-dm.md §2.2`'s own reasoning), disambiguated only by a non-domain, device-level flag that lives entirely outside this table and outside `domain-model.md`. This row's own meaning is unchanged; only the set of real-world journeys that can produce it has grown from one to two.
