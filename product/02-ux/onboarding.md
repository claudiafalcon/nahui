...
- **Settings / capability change after first run.** Whether a Business that chose "Empezar gratis" can later add the NFC entitlement, or whether any capability is ever merchant-self-service-editable at all, is `company/business-decisions.md` Q5 — open, blocking `settings.md`, not this document's to resolve or work around. This is very likely the *most common* real-world path (start free, obtain a kit later) — it is still not designed here. No self-service capability-change mechanism is invented anywhere below.
- **Payments/checkout.** "Activar kit NFC" confirms possession of an already-obtained physical kit (e.g., an activation code) — zero in-app payment processing (`decision-log.md` D19, `company/CLAUDE.md` non-goals). No purchase flow, no price, no card field anywhere in this document.
...
3. Does a Business exist, but Onboarding was left incomplete before §3.5's
   ...
   resolved, e.g. she closed the app mid-way through entering an NFC
   activation code, or while the demo path's confirmation screen, §3.4c,
   was on screen before she chose either option)?
...
|---|---|---|---|---|
| **Empezar gratis** | `{buttons}` | `buttons` (the only value in the set — nothing to resolve, not even a trivial decision) | `free` | `false` |
| **Activar kit NFC** | `{buttons, nfc}` | `nfc` — **set automatically**, no separate question (see 2.3) | `free` | `false` |
...
**Why `subscriptionTier` is `free` for both real paths, always, with no option offered otherwise:** `company/CLAUDE.md`'s business model direction states paid-tier segmentation is "available once user has own sales history" — which structurally cannot exist yet at first run, for any real merchant, regardless of which path she picks. This isn't Onboarding declining to offer a paid option out of caution; there is no honest paid option to offer at this moment, for anyone. A screen that presented "gratis / de paga" as a live choice here would be asking a question with only one truthful answer — itself a small violation of *global-principles.md*'s "never ask twice" in spirit (asking when the system already knows the answer). Upgrading later is exactly the territory Q5 (`company/business-decisions.md`) leaves open and this document does not design around.
...
**Why "Ver un ejemplo" is the one path gated behind an explicit confirmation screen (§3.4c) before its write happens, unlike either real path:** unlike "Empezar gratis" or "Activar kit NFC," tapping this path writes a permanent Business that she can never later turn into her real one (`decision-log.md` D19 — no conversion mechanic) — and, per §2.1, that write starts counting toward Onboarding being permanently "done." The other two paths don't need an equivalent gate because there's nothing to warn her away from: a real Business existing afterward is the whole point of tapping either of them, not a risk to flag. The demo path is different in kind — its entire value is that it *isn't* her real business, and it's the one path where that fact has to land *before* the irreversible write, not only after it. §3.6 Variant C's own honesty line ("no es información real") is real and worth keeping, but it arrives after the write already happened — too late to be the first time she learns this. §3.4c exists to say it once, plainly, at the one point where choosing differently still costs her nothing.

### 2.3 Why "Activar kit NFC" sets `defaultSellingMode = nfc` automatically, never as a separate question

This is the specific ambiguity the task calls out, and it's resolved here, explicitly, not left open:

Choosing "Activar kit NFC" and successfully confirming the kit **is** the statement of intent — she came to this screen specifically to say "I sell with tags." Asking a second, separate question immediately afterward ("¿quieres que las etiquetas sean tu modo normal?") would be asking her to re-answer something she just answered, in more technical language than the path label she already tapped in plainer language. That's a direct violation of two named principles at once:

- *global-principles.md*, "never ask twice" — the system already has the answer the instant she chooses this path.
- *global-principles.md*, "business language before technical language" — `defaultSellingMode` is exactly the kind of raw technical setting D19 says must never be surfaced directly; deriving it silently from her path choice is what keeps it invisible.

This also matches *architecture-principles.md* #1 ("capabilities resolved once, upstream, never asked mid-flow") applied at the very highest point it could possibly apply — before any tab, any Session, any Sale exists. Onboarding is the canonical, purest instance of this principle in the whole product, not an exception to it.

**Composition with NFC Readiness (`decision-log.md` D23) — cross-reference only, no redesign:** a freshly onboarded "Activar kit NFC" Business has `defaultSellingMode = nfc` but, honestly, zero tagged inventory yet — she hasn't received merchandise or assigned a single tag. Her very first real Session will therefore evaluate as **Not Ready** (`home.md` §2/§3.6a): the Session opens in `buttons` automatically, with the one-time, non-blocking mention already fully specified there ("Todavía no tienes prendas con tag para hoy — vas a vender con botones. [Asignar tags]"). This is not a gap Onboarding needs to fix or explain further — it's the two already-specified systems (Onboarding's stable default, Selling's per-Session readiness read) composing exactly as designed. Onboarding's job stops at setting the stable default; `home.md` already owns what happens the moment reality doesn't match it yet.
...
- **Empezar gratis / Activar kit NFC** → Catalog is genuinely empty → Home's cold start (`home.md` §3.3), verbatim, same CTA ("Registrar mercancía") that document already specifies.
...
│                                │
│   [   Activar kit NFC      ]    │  secondary
│   (si ya tienes tu kit en mano)  │  subtext, small
│                                │
...
- "Activar kit NFC" is visually secondary — a real, valid path, but a narrower one (only merchants who already physically hold a kit). The subtext directly beneath it ("si ya tienes tu kit en mano") states that prerequisite at the point of decision, not one screen later — the only reassurance that this path doesn't involve a purchase ("no se te cobra nada aquí," §3.4) previously appeared only after she'd already tapped through, one screen too late for a fact this basic.
...
- Copy never mentions "cuenta gratuita," "suscripción," "modo de venta," or any capability by its technical name — "gratis," "kit NFC," "ejemplo" are the only technical-adjacent words used, and all three are words Ana already has a mental model for.

### 3.4 Activar kit NFC — ingresar código
```
┌───────────────────────────────┐
│ ← Elegir cómo empezar            │
│                                │
│  Activar kit NFC                 │
│  Ingresa el código que viene      │
│  con tu kit.                      │
│                                │
│  [ ____ ____ ____ ]              │
│                                │
│  Esto solo activa el kit que      │
│  ya tienes — no se te cobra       │
│  nada aquí.                       │
│                                │
│  [      Activar       ]          │  disabled until a code is entered
│                                │
│  [ No tengo el código a la mano ] │  escape hatch, tappable
└───────────────────────────────┘
```
- Confirms possession of an already-obtained physical kit — an activation code, never a purchase step, per `decision-log.md` D19's explicit payments/checkout boundary. The reassurance line ("no se te cobra nada aquí") makes that non-goal legible directly in the copy, not just in this document's own reasoning — a deliberate, honest line given how easily "activar" could otherwise be misread as "comprar." This is the first and only place the no-charge reassurance appears — §3.3's own subtext states a related but distinct fact (that she needs to already hold a kit), not the cost point; the two shouldn't be conflated when either screen's copy is revised later.
- "No tengo el código a la mano" routes back to §3.3 with nothing lost and nothing to confirm — she hasn't committed to anything yet at this point, so there's no destructive action to protect against. *global-principles.md*, "never a dead end," same posture every other document's error/recovery states already take.

### 3.4a Validando código — near-instant / slow
```
┌───────────────────────────────┐        ┌───────────────────────────────┐
│  Activar kit NFC                 │        │  Activar kit NFC                 │
│  ▢▢▢▢▢▢▢▢▢▢▢▢ (código atenuado)   │        │        Validando…               │
└───────────────────────────────┘        └───────────────────────────────┘
   near-instant: silent, brief dim              slow (>~1.5s): one plain line
```
- Identical convention to `inventory.md` §3.10 / `events.md` §3.9 / `home.md` §3.8c — silent unless genuinely slow, one calm line, never a technical status string.

### 3.4b Activar kit NFC — código inválido (error)
```
┌───────────────────────────────┐
│ ← Elegir cómo empezar            │
│  Activar kit NFC                 │
...
│  [ ____ ____ ____ ]              │
│  [      Activar       ]          │
│  [ No tengo el código a la mano ] │
└───────────────────────────────┘
```
- Plain-language error, no technical validation detail (no "checksum failed," no raw code echoed back distorted). *global-principles.md*, "business language before technical language."
- "No tengo el código a la mano" stays reachable here too — an invalid code is exactly the moment she might want to abandon this path for now, not get stuck retyping indefinitely.
...
- "Mejor quiero registrar mi negocio real" routes back to §3.3 with nothing lost and nothing to confirm, identical posture to "No tengo el código a la mano" (§3.4) — she hasn't committed to anything at this point, so there's no destructive action to protect against. *global-principles.md*, "never a dead end."
...
└───────────────────────────────┘
```
- Nothing about her choice (which path, a validated NFC code, or a confirmed demo) is lost by a failed creation — retrying replays the same, already-confirmed choice, never asks her to redo the path selection, re-enter a code that already validated, or re-confirm the demo path's §3.4c screen. *global-principles.md*, "the best interface stays out of the merchant's way," same guarantee `inventory.md` §3.11 makes for a failed Guardar mercancía.
...
**Variant B — Activar kit NFC:**
```
┌───────────────────────────────┐
│                                │
│         Todo listo              │
│                                │
│  Tus tags ya están activados.    │
│  En cuanto etiquetes tu           │
│  mercancía, vas a poder vender    │
│  con ellos.                       │
│                                │
│      [     Empezar      ]       │
│                                │
...
From §3.3, tap a path:
  ...
  Activar kit NFC
    → ingresar código (3.4)
    → tap Activar → validando (3.4a)
        → inválido (3.4b) → reintentar, o "No tengo el código a la mano" → back to 3.3
        → válido → creando tu negocio (3.5) → error (3.5a) → Reintentar
...
4. Activar kit NFC — ingresar código
5. Activar kit NFC — validando código (near-instant / slow)
6. Activar kit NFC — código inválido (error)
7. Ver un ejemplo — confirmar antes de continuar
8. Creando tu negocio (near-instant / slow) — shared by all three paths
9. Creando tu negocio — error
10. Todo listo — Variant A (Empezar gratis)
11. Todo listo — Variant B (Activar kit NFC)
12. Todo listo — Variant C (Ver un ejemplo)
13. Retomar onboarding interrumpido (resumes any of states 3–9, pixel-identical, in-progress data intact; if interrupted while a Todo listo variant (10–12) was showing, resumes that exact variant instead of re-running the write or restarting from state 3)
14. Falla defensiva — no se pudo determinar el estado inicial
...
| Activar kit NFC, valid code first try | **2** (path + Activar) + typing the code (+1 optional milestone tap) | The code is a real, necessary fact — confirmation of an already-obtained physical kit, not padding. Can't be inferred or skipped without breaking the payments/checkout boundary (`decision-log.md` D19). |
...
Every scenario's floor is dominated entirely by the information that scenario genuinely requires (a code, for the one path where a code is the actual point) — none of the three paths carries a single tap that exists only for ceremony, except two deliberate exceptions: the milestone screen (§3.6), which is itself skippable via auto-continue, and the demo path's one confirmation tap (§3.4c), which is deliberately *not* skippable — because the fact it discloses has to be seen, not skimmed past, before an irreversible write.
...
- `subscriptionTier` resolving silently to `free` for both real paths — never presented as a live choice, since `company/CLAUDE.md`'s business model makes `paid` structurally impossible to honestly offer at first run (§2.2).
- `defaultSellingMode` deriving automatically from path choice (`nfc` for Activar kit NFC, `buttons` for Empezar gratis) — never a second, separate technical question (§2.3).
...
4. **The most likely real-world capability-change scenario is explicitly not designed here:** a merchant who chooses "Empezar gratis" today and obtains a physical NFC kit weeks or months later has no path in this document to add the `nfc` entitlement to her already-onboarded Business — that's precisely `company/business-decisions.md` Q5 (open), which blocks `settings.md`. This document does not invent a workaround; it's named here explicitly so the gap stays visible rather than silently assumed away.

## 9. Principle justification

**global-principles.md:**
- *"The fastest interaction is the one that never happens"* — Bienvenida and the path choice are merged into one screen instead of two (§3.3, §10); Empezar gratis has a 1-tap floor; `subscriptionTier` is never presented as a choice at all, since there's only one honest answer at first run (§2.2); the milestone screen (§3.6) auto-continues rather than requiring a tap. **The one deliberate exception to this principle anywhere in this document is the demo path's confirmation tap (§3.4c)** — added on purpose, not left over by accident, because the fact it discloses (an irreversible, non-convertible write) has to reach her before she commits; a tap that exists specifically to slow her down for a fact this consequential isn't the kind of waste this principle argues against.
- *"Never ask twice"* — `defaultSellingMode` is never asked as a separate question after "Activar kit NFC" already answered it (§2.3); Onboarding is never shown again once complete, including the §3.6 milestone actually being seen (§2.1, D13); an interrupted flow resumes at the exact step, never re-asking anything already entered (§3.7).
...
- *"Business language before technical language"* — every screen uses "gratis," "kit NFC," "ejemplo," "activar," never "registrationMode," "subscriptionTier," "defaultSellingMode," or "capability," anywhere, including in the error and confirmation states (§3.4b, §3.4c, §3.5a).
...
- *"Capture business truth once, reuse it forever"* — the NFC activation code (a real fact: she possesses a physical kit) is confirmed exactly once and never re-entered on retry after a creation failure (§3.5a); the demo path's confirmation (§3.4c) is likewise never re-asked on a retried creation.
- *"The best interface stays out of the merchant's way"* — a failed Business-creation write never drops an already-validated code, already-chosen path, or already-confirmed demo screen (§3.5a); "No tengo el código a la mano" and "Mejor quiero registrar mi negocio real" are real, always-available escape hatches, never dead ends (§3.4, §3.4c).
...
- **"Activar kit NFC" carries a one-line prerequisite subtext directly on its §3.3 button** ("si ya tienes tu kit en mano") — moved forward from §3.4, where an earlier draft's only related reassurance ("no se te cobra nada aquí") appeared one screen after she'd already committed to tapping through.
...
- **`defaultSellingMode` is set automatically from the path choice, with no separate confirmation question, for "Activar kit NFC"** (§2.3) — resolves the ambiguity named explicitly in this task's brief. Her path choice *is* the statement of intent; a second question would violate "never ask twice" and "business language before technical language" at once.
- **`subscriptionTier` is `free` for both real paths, with no paid option ever presented at Onboarding** (§2.2) — not a conservative default chosen among options, but the only honest value available, since `company/CLAUDE.md`'s business model gates paid tier behind sales history that cannot exist at first run.
...
- Adding the `nfc` entitlement to an already-onboarded, previously-free Business (the "bought a kit later" scenario) — blocked entirely on `company/business-decisions.md` Q5; revisit this document only once Q5 resolves and `settings.md` exists.
- The exact format/length of the NFC activation code (§3.4) is left illustrative here — a Builder-level detail, not a UX decision this document needs to fix.
