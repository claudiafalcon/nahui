...
**Only offered at all when `nfc ∈ registrationMode`** (`decision-log.md` D71; corrected 2026-09-17, `decision-log.md` D73 — the second clause D72 briefly added, `defaultSellingMode !== 'nfc'`, is dropped). D72's reasoning for that second clause — "whole-catalog `nfc` mode already sells everything ready-tagged automatically (D23's Ready branch), so there's nothing for this per-Product opt-in to compose with until she's back in `buttons` mode" — is named a category error by D73: it conflated Selling-mode *composition* (`home.md`'s own concern, which mode a Session resolves toward) with Inventario tagging-*preparation* (this capability's actual concern, which Products are eligible to have their units tagged at all). The two are independent facts about a Business, not mutually exclusive ones. Combined with D72's own already-live retirement of the `buttons → nfc` write path, keeping the second clause would have left `nfcPerProductEnabled` permanently unreachable for any Business that stays in `nfc` mode — no self-service way in or out of it, ever — a real, live account hit exactly this gap. The gate is now the same `subscriptionTier = paid`-derived condition that already decides whether `defaultSellingMode`'s `nfc` option exists at all (§2.3) — nothing more. A Free-tier or otherwise non-NFC-capable Business still never sees this row, anywhere in this document — no locked/disabled hint, no upsell copy explaining what she'd get by upgrading — the identical posture §2.3 already reasons through for a disabled-but-visible `nfc` option and §2.7 already applies to "Tu equipo." Discoverable, same as those two, only via "Activar plan de pago"'s own copy and by becoming Paid.
...
**Resolved, 2026-09-17 (`decision-log.md` D73) — no longer an edge case.** A Business with `nfcPerProductEnabled = true` while `defaultSellingMode` still reads `nfc` is now simply valid, ordinary, fully-supported data, the same as any other Business — nothing left needing design. The corrected gate above (`nfc ∈ registrationMode` alone) means "Activar NFC"/"Desactivar NFC" is visible and usable from the `nfc`-moded vista principal itself (§3.3a), alongside the escape valve, with no need to switch to `buttons` mode first to reach it.
...
**Paid-tier state, `defaultSellingMode = nfc` (grandfathered/demo only — corrected 2026-09-17, `decision-log.md` D72; further corrected same day, `decision-log.md` D73):**
```
┌───────────────────────────────┐
│ ← Hoy                          │
│  Configuración                  │
...
`nfcPerProductEnabled = true` — identical toggle-label behavior to the
`buttons`-mode state above.)

**Corrected 2026-09-17 (`decision-log.md` D73) — the "Activar NFC"/"Desactivar NFC" row now shows here too, same row shape as the `buttons`-mode state above, alongside the escape valve.** D72's original reasoning for suppressing it in this state (a "nothing to compose with" argument) is a category error D73 names explicitly — see §2.8. She no longer has to switch to `buttons` mode first to reach the per-Product NFC control; both actions are available in the same visit, exactly as they'd be for any other Paid-tier Business. Reached by any Paid-tier merchant whose `defaultSellingMode`
...
└───────────────────────────────┘
```
The grandfathered/demo `nfc`-mode equivalent of this pending-change state mirrors §3.3a's `nfc`-mode state exactly (corrected 2026-09-17, `decision-log.md` D73) — "Con tags" + "Cambiar a vender con botones," plus the "NFC: No"/"Activar NFC" (or "Desactivar NFC") row — with §3.6's own pending-change banner substituted for §3.3a's plain "Tu plan" line; not redrawn separately here, per this folder's own shared-state discipline.
...
From the main view, tap any action row:
  ...
  Activar NFC por producto / Desactivar NFC por producto
    → confirmación de efecto inmediato (§3.4) → {acción} ahora
        → guardando (§3.9) → error (§3.10) → Reintentar
        → success → back to vista principal, row updated, no pending
          state, no handoff — see §2.8 (this action never routes
          anywhere beyond Configuración's own vista principal, unlike
          "Cambiar a vender con tags")
...
5. Confirmación de efecto inmediato — generic template (five copy variants: Activar plan de pago — SET-M4/D34/D40-corrected; the two `defaultSellingMode` variants; the two new `nfcPerProductEnabled` variants, `decision-log.md` D71; "Activar/Desactivar clientes frecuentes" retired entirely per `decision-log.md` D40)
...
| Activar NFC por producto | 2 (Activar NFC por producto → Activar ahora) | Same two-tap floor as every other immediate-effect action — nothing to type, a pure capability flip. |
| Desactivar NFC por producto | 2 | Same floor — the consequence disclosed in §3.4's copy costs no extra tap, only a longer read. |
...
- Whether "Activar NFC por producto" is offered at all — computed automatically from `nfc ∈ registrationMode`, the identical derivation discipline `defaultSellingMode`'s own `nfc` option already uses (§2.3/§2.8) — never something Ana unlocks separately.
