...
**Further amended 2026-09-17 (`decision-log.md` D73 — composed NFC-tagging-eligible test drops its first disjunct; `nfcPerProductEnabled = true` while `defaultSellingMode = 'nfc'` becomes ordinary, valid data):** §2.8's own gate on "Activar NFC"/"Desactivar NFC" drops its `defaultSellingMode !== 'nfc'` clause — offered whenever `nfc ∈ registrationMode` alone, matching §2.2's table. D73 names the retired clause's old reasoning ("whole-catalog `nfc` mode already sells everything ready-tagged automatically... nothing to compose with") a category error, conflating Selling-mode composition (`home.md`'s own scope) with Inventario tagging-*preparation* (this document's actual concern) — a barcoded Product being swept into "needs an NFC tag" via `inventory.md`'s own now-corrected composed test (that document's own D73 amendment) was never a state that could actually resolve, since `Product.barcode` and `Product.nfcTaggingEnabled` are server-side mutually exclusive (D71). Combined with D72's already-live `defaultSellingMode: buttons → nfc` retirement, the old gate left `nfcPerProductEnabled` permanently unreachable for any Business that stays in `nfc` mode — a real, live account hit exactly this. §2.8's own "Edge case, named not solved" paragraph is resolved, not merely narrowed: a Business with `nfcPerProductEnabled = true` while `defaultSellingMode = 'nfc'` is now simply valid, ordinary, fully-supported data, the same as any other Business — there's nothing left needing design. §3.3a's `defaultSellingMode = nfc` vista-principal wireframe is corrected to show both the "Cambiar a vender con botones" escape valve and the "Activar NFC"/"Desactivar NFC" row, in the same shape the `buttons`-mode state already uses; §3.6's matching pending-change mirror corrected to match. `ux-critic`/`reviewer` review pending, not skipped — same live-urgency posture as D71/D72.

**Further amended 2026-09-18 (`decision-log.md` D79, `product/99-rfc/0017-nfc-composable-selling-capability.md`, Accepted — NFC becomes a live, composable selling capability; `Session.operatingMode`/NFC Readiness retired outright; `Business.defaultSellingMode` fully retired):** the Product Owner's own explicit instruction on this exact surface: "I do not want two separate concepts... remove the idea of 'Cómo vendes normalmente'... there should simply be an NFC capability setting." **"Cómo vendes normalmente" (§2.3's status line/action, and every wireframe row it produced) is retired in full, both directions** — not narrowed to the one-way escape valve D72 left standing. D79 fully retires `Business.defaultSellingMode` itself, including the "Settings write action" §2.3 still carried (its one-way `nfc → buttons` escape valve) — there is zero remaining functional reader of this field anywhere in the Foundation, since the composable surface it once gated (`home.md` §3.10) is itself retired. **§2.8's "Activar NFC"/"Desactivar NFC" becomes the sole NFC-related control in this document, unconditionally** — every Paid-tier Business, regardless of its historical `defaultSellingMode` value, sees the identical single row. §2.6 (the already-dead "Cambiar a vender con tags" handoff mechanism, marked historical since D72) is left exactly as marked, with one addition: its own premise is now doubly moot, since the field it wrote no longer has any reader to hand off *to*. §1/§2.2/§2.3/§3.3a/§3.6/§4/§5/§6/§7/§9/§10 all corrected to match — see each section's own edit below. **`ux-critic`/`reviewer` review pending, not skipped — same live-urgency posture as tonight's other D71/D72/D73 passes.**
...
### 2.2 The three actions, their templates, and today's illustrative immediate/deferred assignment

**Retitled 2026-09-18 (`decision-log.md` D79) — three actions across two capabilities, not four across three.** `defaultSellingMode` is fully retired — its row, and the one-way escape-valve action it produced, are struck below.
...
| ~~`defaultSellingMode`~~ | ~~nfc → buttons~~ | ~~Cambiar a vender con botones~~ | — | **Retired 2026-09-18, `decision-log.md` D79.** This row's own one-way escape valve (added D72) is itself retired — `defaultSellingMode` has zero remaining functional readers anywhere in the Foundation, since the whole-catalog `nfc` Selling surface it once gated (`home.md` §3.10) is itself retired. There is no longer anything for this action to "escape" from. |
| `nfcPerProductEnabled` | false → true | **Activar NFC** | §3.4 (immediate-effect) | Immediate — offered only when `nfc ∈ registrationMode` (`decision-log.md` D71, corrected `decision-log.md` D73). Off by default even on an already-NFC-capable Business — a deliberate opt-in, never automatic. |
| `nfcPerProductEnabled` | true → false | Desactivar NFC | §3.4 (immediate-effect) | Immediate. |

**`nfcPerProductEnabled` is the sole NFC-related capability in this document, for every Business, unconditionally — corrected 2026-09-18 (`decision-log.md` D79, the Product Owner's own explicit instruction: "I do not want two separate concepts... there should simply be an NFC capability setting").** D72 had already collapsed two controls into one for a `buttons`-mode Business specifically; D79 finishes that collapse by retiring the field the second control depended on entirely, so there is no longer a `buttons`-mode/`nfc`-mode distinction of any kind for this row to be conditional on. Same field, same RPC, same semantics D71 already established. It doesn't replace or narrow anything `subscriptionTier` controls; it's a genuinely new opt-in, off by default even for an already-Paid Business.

**`registrationMode`'s `nfc` entitlement is no longer its own row in this table, and no longer has a dedicated action anywhere in this document** (`decision-log.md` D27, superseding the relevant part of D25). It has no independent existence to toggle: `nfc ∈ registrationMode` is a pure read-time derivation from `subscriptionTier = paid`, so it changes automatically, as a side effect, whenever "Activar plan de pago" or "Volver al plan gratis" changes `subscriptionTier` — never through an action of its own. The earlier design's dedicated NFC-activation path (an activation code confirming a physical kit) is removed entirely, not merely relocated: D27 corrected the underlying assumption that kit possession was ever the thing granting the capability. **What she can control directly is `nfcPerProductEnabled` alone (§2.8) — `defaultSellingMode`'s former escape-valve direction is retired, `decision-log.md` D79; see §2.3 for the full reasoning.**
...
Every action honors D25's two invariants identically: **never delete historical data**, and **business rules determine timing**.

**"Cambiar a vender con tags" — fully retired, not merely dormant, as of `decision-log.md` D79.** See §2.6, kept as historical record only.

### 2.3 "Cómo vendes normalmente" — retired in full, `decision-log.md` D79 (previously: why `defaultSellingMode`'s one remaining direction was immediate, D27, narrowed to one-way by D72)

**Retired in full, 2026-09-18 (`decision-log.md` D79) — the Product Owner's own explicit instruction: "I do not want two separate concepts... remove the idea of 'Cómo vendes normalmente'... there should simply be an NFC capability setting."** `Business.defaultSellingMode` is fully retired at the Foundation level — kept in schema as inert historical data on any already-existing row (never deleted, D25), no code path reads or writes it going forward. This section's whole subject — a status line/action distinct from §2.8's "Activar NFC" — no longer exists in any form, for any Business, regardless of its historical `defaultSellingMode` value. §2.8 is now the sole NFC-related control in this document (see §2.8's own corrected text).

Kept below, struck rather than deleted, per this document's own non-deletion discipline for a fully superseded mechanism — historical record only, not a live spec. Full prior content also mirrored at `settings.changelog.md#section-2-3-cómo-vendes-normalmente-retired-d79`.

Unlike `subscriptionTier`, `defaultSellingMode` was never a commercial or billing capability — it was an operational fallback (`decision-log.md` D23) that Session-start once read fresh, every time, alongside NFC Readiness (both retired, D79). `decision-log.md` D25's deferred-timing rationale existed specifically to make a *commercial* change (one with billing-cycle implications, like a Paid→Free downgrade) honestly displayable before it lands; `defaultSellingMode` carried no such implication in either direction.

This was why the one remaining direction — Etiquetas NFC → Botones — used the generic immediate-effect template (§3.4), never the deferred one (§3.5): there was nothing to defer. The reverse direction no longer existed as a merchant-facing action at all as of `decision-log.md` D72; `decision-log.md` D79 then retired the surviving direction too. The change, while live, took effect at her very next Session-open, automatically, with no separate scheduling mechanism and no pending-value/effective-date pair to track (`decision-log.md` D27 stated this explicitly).

**Corrected 2026-09-17 (`decision-log.md` D72), then superseded 2026-09-18 (`decision-log.md` D79) — this control stopped being a two-way picker for any Business (D72), then stopped existing at all (D79).** A Business in `buttons` mode previously saw "Cómo vendes normalmente: Botones" as a plain, current-state line with no action beside it. A Business already `nfc`-moded (grandfathered, or demo-seeded per `onboarding.md` §2.2, D19's own contingency) previously saw this row as a real control: "Con tags," with a single available action, "Cambiar a vender con botones." As of D79, neither line renders for either population — §3.3a/§3.6's wireframes are corrected to show only "Tu plan" and "Activar NFC"/"Desactivar NFC," identically for every Paid-tier Business regardless of its historical `defaultSellingMode` value.

**`defaultSellingMode`'s stored value was never written by any action other than its own former row above, while it was live.** "Volver al plan gratis" (§2.2, §3.5) writes only to `subscriptionTier` — it never touched `defaultSellingMode`, and still doesn't; there's simply no field left for it to touch that anything reads.
...
### 2.6 What "Cambiar a vender con tags" did beyond flipping the field —
hand off into tagging, or guide her to register first (historical record —
architect ruling, `decision-log.md` D46 Addendum; the field itself fully retired, `decision-log.md` D79)

**Retired as a live merchant action, 2026-09-17 (`decision-log.md` D72) — kept below as a historical/mechanism record, not currently reachable from any Settings screen.** "Cambiar a vender con tags" itself no longer exists (§2.2/§2.3) — D72 retired the `buttons → nfc` direction outright, with no merchant-facing way to trigger it going forward. Everything below this note describes a handoff mechanism that fired only from that now-retired action; it cannot fire again for any Business that reaches `buttons` mode the ordinary way (every real Onboarding path, `onboarding.md` §2.3). The one population that ever read `defaultSellingMode = 'nfc'` — a grandfathered/demo Business — never ran through this handoff either, since none of them arrived at `nfc` via this action.

**Finished, 2026-09-18 (`decision-log.md` D79) — this section's own premise is now doubly retired, not merely the action that once triggered it.** `Business.defaultSellingMode`, the field this section's every bullet describes writing/reading, is itself fully retired at the Foundation level — zero functional readers remain anywhere, confirmed by trace (`decision-log.md` D79). `inventory.md`'s own §2 step 0 trigger condition, keyed to the entry marker this section used to hand off, was already marked "dormant, D72; doubly dead, D73" in that document — its status is unchanged by this correction, since D73 had already made the trigger's own premise false independent of D72's reachability question. Nothing further to correct in `inventory.md` as a result of this specific field's Foundation-level retirement — see that document's own D79 pass for its own, separate corrections.
...
### 2.8 What "Activar NFC" / "Desactivar NFC" do (renamed from "…por producto," and the single, sole NFC-related control in this document — `decision-log.md` D71, D72, D79, `product-decisions.md` Q31)

**Renamed from "Activar NFC por producto"/"Desactivar NFC por producto," 2026-09-17 (`decision-log.md` D72).** The field this writes, `Business.nfcPerProductEnabled`, and the RPC it calls, are unchanged — same `change_nfc_per_product_enabled` write, same semantics D71 already established. "Por producto" is dropped because there's no longer a second, whole-catalog NFC control for it to disambiguate against.

**Confirmed as the sole NFC-related control, 2026-09-18 (`decision-log.md` D79) — not merely the survivor of D72's collapse.** D72 had collapsed two controls into one *for a `buttons`-mode Business specifically*, while still leaving §2.3's one-way escape valve standing for a grandfathered/demo `nfc`-mode Business. D79 retires `defaultSellingMode` itself, so there is no longer any population, grandfathered or otherwise, that sees a second NFC-related row of any kind — this action is now unconditionally the only one, for every Paid-tier Business.

**Only offered at all when `nfc ∈ registrationMode`** (`decision-log.md` D71; corrected 2026-09-17, `decision-log.md` D73 — the second clause D72 briefly added, `defaultSellingMode !== 'nfc'`, is dropped). D72's reasoning for that second clause — "whole-catalog `nfc` mode already sells everything ready-tagged automatically... so there's nothing for this per-Product opt-in to compose with until she's back in `buttons` mode" — is named a category error by D73: it conflated Selling-mode *composition* (`home.md`'s own concern) with Inventario tagging-*preparation* (this capability's actual concern). The two are independent facts about a Business, not mutually exclusive ones. **This distinction is now moot at the Foundation level, not just resolved in this document's own gate, `decision-log.md` D79 — there is no longer a "whole-catalog `nfc` mode" for anything to compose with or against; every Session composes buttons/barcode/NFC identically, unconditionally.** The gate is the same `subscriptionTier = paid`-derived condition (`registrationMode`, D27/D79) — nothing more. A Free-tier or otherwise non-NFC-capable Business still never sees this row, anywhere in this document — no locked/disabled hint, no upsell copy explaining what she'd get by upgrading — the identical posture §2.7 already applies to "Tu equipo." Discoverable, same as that, only via "Activar plan de pago"'s own copy and by becoming Paid.
...
**Turning it ON: immediate effect, same generic template as `defaultSellingMode` (§3.4), no pending-value/effective-date structure** — same mutability class `decision-log.md` D71 states explicitly ("same class as `defaultSellingMode`"), for the same reason §2.3 already gives `defaultSellingMode` no deferred structure: this isn't a commercial/billing capability, so there's nothing to defer. **What actually changes, stated plainly before she confirms (§3.4's own copy):** a new per-Product toggle becomes visible in `inventory.md`'s Catalog row — but only on a row with no `Product.barcode` assigned (`inventory.md` §3.4's own new amendment, cross-referenced, not redescribed here). This action itself never routes her anywhere new — unlike "Cambiar a vender con tags" (§2.6), turning this on doesn't hand off into any Inventario resolution or create an actionable queue by itself; it only makes the per-Product choice available, waiting for her to decide, per Product, whether to opt in. No inventory is even read or touched by this write.

**Turning it OFF: immediate effect, same template, with a real, honest consequence to disclose — never a silent flip.** Two things stay true, stated explicitly, not left implicit:
1. **No already-tagged `InventoryUnit` is ever untagged or orphaned.** `Product.nfcTaggingEnabled` values already set on individual Products aren't reset or cleared by this action — the same "two independent stored fields, no reset rule needed" pattern §2.3 already establishes between `defaultSellingMode` and `subscriptionTier`. An already-tagged Camisas unit stays exactly as tagged as it was, and stays sellable via NFC — unit-level sellability is a pure `InventoryUnit.tagId` read, per D71, never something this Business-level flag can revoke retroactively. What stops is only *future* eligibility: `inventory.md`'s own Asignar Tags mechanism stops offering any not-yet-tagged unit for tagging, for any Product, from the moment this lands.
...
**Resolved, 2026-09-17 (`decision-log.md` D73), then simplified further, 2026-09-18 (`decision-log.md` D79) — no longer an edge case, and no longer even a distinct case.** D73 first made a Business with `nfcPerProductEnabled = true` while `defaultSellingMode` still read `nfc` simply valid, ordinary data. D79 goes further: `defaultSellingMode` itself is retired, so there is no longer an `nfc`-moded vista principal at all for this row to appear "alongside the escape valve" within — §3.3a shows exactly one wireframe shape for every Paid-tier Business, with "Activar NFC"/"Desactivar NFC" as its only NFC-related row.
...
└───────────────────────────────┘
```
No NFC control shown while `subscriptionTier=free` — nothing to activate yet, since `nfc` isn't in her capability set (§2.8). **Corrected 2026-09-18 (`decision-log.md` D79) — "this row gains a real control" no longer refers to a `defaultSellingMode` picker (retired); it refers to §2.8's "Activar NFC" row, which is the only NFC-related row that ever appears, on the single Paid-tier shape below.**

**The "Tu cuenta" section is new (2026-08-13, §2.5) and appears identically in every vista-principal variant below** — the Paid-tier state and §3.6's pending-change state — regardless of `subscriptionTier` or pending-change status, the one part of this screen never conditioned on anything else in it. **Corrected 2026-09-18 (`decision-log.md` D79) — the separate `defaultSellingMode = nfc` mirror this bullet used to also name is retired; the Paid-tier state below is now the only Paid-tier shape.**
...
**Paid-tier state — one shape, for every Business, regardless of `defaultSellingMode`'s historical value (corrected 2026-09-18, `decision-log.md` D79 — collapses the two prior variants below into one):**
```
┌───────────────────────────────┐
│ ← Hoy                          │
│  Configuración                  │
...
└───────────────────────────────┘
```
(NFC row shows "Desactivar NFC" instead of "Activar NFC" once
`nfcPerProductEnabled = true`.)

**"Tu equipo" (new, §2.7 — `product-decisions.md` Q24/Q25), present on every Paid-tier vista principal, absent entirely on the Free-tier vista principal.** Count line reads "2 personas vendiendo contigo" from `BusinessMembership.status = active` rows only — a pending invitation or a revoked row doesn't count toward it. Zero active members reads "Nadie más vendiendo contigo todavía," never a bare "0." Opens §3.11.

**This single shape retires the two prior variants that used to distinguish a `buttons`-mode Business from an `nfc`-moded (grandfathered/demo) one — 2026-09-18, `decision-log.md` D79.** `Business.defaultSellingMode` is fully retired, along with §2.3's "Cómo vendes normalmente" line and its escape-valve action — there is no longer any wireframe distinction, of any kind, driven by that field's historical value. Every Paid-tier Business, grandfathered/demo or ordinary, sees this exact shape.
...
**Copy variant — Cambiar a vender con botones — retired entirely, 2026-09-18 (`decision-log.md` D79).** This copy variant no longer exists anywhere in this document — D79 fully retires `defaultSellingMode`, including the one-way escape valve D72 had left standing. Kept here as a one-line record: D27 first introduced the two-way control; D72 narrowed it to this one surviving direction; D79 retires the field and this direction along with it, since there is no longer an `nfc`-mode Selling surface (`home.md` §3.10, retired) for any Business to need an escape valve from.
...
todavía, tú decides cuáles en Inventario.
[ Activar ahora ]
```
...
y se puede seguir vendiendo con NFC sin problema.
[ Desactivar ahora ]
```
...
└───────────────────────────────┘
```
**Corrected 2026-09-18 (`decision-log.md` D79) — this is now the one shape for every Paid-tier Business with a pending change, regardless of `defaultSellingMode`'s historical value.** The distinct grandfathered/demo `nfc`-mode mirror this section used to reference is retired along with `defaultSellingMode`/§2.3 — see §3.3a's own corrected text for the full reasoning; not restated here, per this folder's own shared-state discipline.
...
- **This constraint binds every reuse of this mechanic, not only this document — and here is exactly where the citation edge does and does not exist (corrected 2026-09-21, `ux-critic` Major 2).** Three documents cite this section by name and carry the constraint in their own words, so it genuinely binds there: `inventory.md` §3.19, `home.md` §3.8c, `reports.md` §3.18. **Three others do not cite this section at all** — `inventory.md` §3.10, `events.md` §3.9 and `onboarding.md` §3.5 cite each other laterally, and their citation graph never reaches here. **That is harmless today for a specific reason, stated so it is not mistaken for an oversight: none of the three attenuates anything.** All three draw a silent *skeleton* (`▢▢▢▢` placeholder blocks in an otherwise empty frame), so there is no dim for a status word to fall inside and nothing for them to inherit. **The rule for the future is therefore explicit: any state in those documents that introduces an attenuation comes back to this section first.** An earlier draft of this bullet claimed all six cite this section, which was false for half of them — and a sentence that manufactures false confidence about where a rule reaches is the same failure that let the original defect through. `inventory.md` §3.19's NFC row is the case that makes it concrete — its control's attempted position is required to be perceivable *at exactly the moment* the row is quieted, which a row-wide attenuation defeats. That requirement is already approved there ("the control's position and the word never disagree, in any state"); this section states its general form so the next reuse doesn't have to rediscover it.
- **The general test, for any reuse — when a dim is legitimate at all.** A dim is legitimate when everything under it is content she has already read and cannot act on until the write resolves. It stops being legitimate the moment something under it **(a)** states the write's status, **(b)** holds a position she attempted, or **(c)** is the retry target. A state that dims a surface still carrying an instruction she needs *during* the write is a different design problem and comes back here as one — it is never solved by raising the contrast of a dim. *(Recorded here, the pattern's home, 2026-09-21; it was first written into `reports.md` §3.18, which is where a future reader would not look for it.)*
  - **Geometry usually decides this before the rule has to.** Where the dim and the status word occupy the same region — one row, as at `inventory.md` §3.19 — scoping is unavoidable and this rule does the work. Where the status word renders in a foreground layer over a dimmed backdrop, as at `reports.md` §3.18, the separation is structural and the word was never at risk. **A full-screen dim is therefore the safest of the three shapes, not the most dangerous** — the defect class was never "dims are bad," it was "the dim swallowed the one new element on screen."
...
From the main view, tap any action row:
  ...
  [Retired 2026-09-18, `decision-log.md` D79 — "Cambiar a vender con tags" and "Cambiar a vender con botones" no longer exist as merchant-facing actions; `defaultSellingMode` has zero remaining functional readers. See §2.3/§2.6's own historical record for what this flow used to do.]
  ...
  Activar NFC / Desactivar NFC
    → confirmación de efecto inmediato (§3.4) → {acción} ahora
        → guardando (§3.9) → error (§3.10) → Reintentar
        → success → back to vista principal, row updated, no pending
          state, no handoff — see §2.8
...
5. Confirmación de efecto inmediato — generic template (three copy variants: Activar plan de pago — SET-M4/D34/D40-corrected; the two `nfcPerProductEnabled` variants, `decision-log.md` D71. **Narrowed from five to three, 2026-09-18 (`decision-log.md` D79)** — the two `defaultSellingMode` variants are retired along with the field; "Activar/Desactivar clientes frecuentes" was already retired per `decision-log.md` D40.)
...
| ~~Cambiar a vender con botones~~ / ~~Cambiar a vender con tags~~ | — | **Retired 2026-09-18, `decision-log.md` D79** — `defaultSellingMode` is fully retired; neither action exists any longer. |
...
- ~~Which `defaultSellingMode` direction is offered~~ — **Retired 2026-09-18, `decision-log.md` D79.** No such computation exists any longer; `nfc ∈ registrationMode` now governs only whether §2.8's own single row is offered (see the corrected bullet below).
...
- ~~Whether "Cambiar a vender con tags" hands off into tagging...~~ — **Retired 2026-09-18, `decision-log.md` D79.** This action no longer exists (§2.6, historical record only).
- Whether "Tu equipo" appears in the vista principal at all — a pure read of `subscriptionTier`, same derivation discipline `nfcPerProductEnabled`'s own gate already uses (§2.7/§2.8).
...
- Whether "Activar NFC" is offered at all — computed automatically from `nfc ∈ registrationMode` (§2.8) — never something Ana unlocks separately.
- Whether an already-tagged `InventoryUnit` stays sellable via NFC after `nfcPerProductEnabled` turns off — a pure `InventoryUnit.tagId` read, per `decision-log.md` D71, never something this Business-level flag revokes or this document computes itself.
...
5. **Resolved, then the field itself retired — kept for continuity.** `decision-log.md` D27 first extended self-service editability to `defaultSellingMode` (§2.2/§2.3/§3.4/§3.6, as it stood at the time). `decision-log.md` D79 (2026-09-18) retires `Business.defaultSellingMode` at the Foundation level, along with every action/row this document once built for it — see §2.3's own status header for the full record. This item stays marked resolved rather than reopened: the gap it originally flagged (a field D25/Q5 hadn't covered) was genuinely closed by D27, and its subsequent retirement is a separate, later Foundation correction, not a reopening of this specific scope question.
...
19. **This document's own §2.8 disclosure of the "Leer con NFC" overlay consequence describes a mechanism designed in `home.md`'s own, parallel D71 amendment, not this document.** If that amendment hasn't landed at the moment this ships, the disclosure describes a real but not-yet-buildable consequence — not a gap in this document, flagged so the cross-document dependency is visible rather than silently assumed.

## 9. Principle justification

**global-principles.md:**
- *"Never ask twice"* — a pending change's target capability is never re-asked when cancelling; the landing acknowledgment (§2.4) is shown exactly once. **`defaultSellingMode` is retired outright, 2026-09-18 (`decision-log.md` D79)** — there is no longer a Session-start read for anything set here to feed; `home.md`'s own §2/§3.9 now read only `nfcPerProductEnabled`, live, on every render.
- *"Business language before technical language"* — every screen uses "plan," "NFC," never `subscriptionTier`, `registrationMode`, `nfcPerProductEnabled`, or "entitlement," anywhere. `defaultSellingMode`/"cómo vendes" copy is retired along with the field, `decision-log.md` D79. "Clientes frecuentes" no longer appears as an action name in this document (`decision-log.md` D40) — only as a description of what "Activar plan de pago"/"Volver al plan gratis" affect.
...
- *"The best interface stays out of the merchant's way"* — a failed capability save never drops an already-confirmed toggle; cancelling a pending change is always reachable and never destructive to anything but the pending change itself (§3.7).
...
- *"Never delete historical data"* (D25) — extended to `nfcPerProductEnabled`'s off-direction (§2.8, `decision-log.md` D71): an already-tagged `InventoryUnit` is never untagged or orphaned by this flip; only future eligibility stops.
...
- *#6 (one-way dependency direction)* — this document only writes to Identity's Business Capabilities; it never designs a Selling/Inventory/Intelligence screen of its own. "Cerrar sesión" writes only to this device's own session fact, not part of the domain model at all (RFC 0007 §5's explicit infrastructure deferral) — it never reads or writes Selling, Inventory, or Intelligence data, the identical discipline the rest of this document already holds itself to. **"Cambiar a vender con tags" (§2.6, now retired in full per `decision-log.md` D79) observed this identically while live** — its D46-Addendum-corrected design wrote `defaultSellingMode` and handed off a bare entry marker, never reading or querying `InventoryUnit`/Catalog state itself. **`nfcPerProductEnabled`'s own write path never reads Inventory-owned state to decide anything (§2.8)** — this capability has no dependency-cycle risk to guard against at all, stated explicitly rather than assumed.
...
- **A `defaultSellingMode` control (Botones ↔ Etiquetas NFC) was added** (`decision-log.md` D27), later narrowed to a one-way escape valve (D72), then retired in full, 2026-09-18 (`decision-log.md` D79) — the Product Owner's own explicit instruction to remove "Cómo vendes normalmente" as a concept, with `nfcPerProductEnabled` (§2.8) standing as the sole NFC-related control going forward. **[see settings.changelog.md#decisions-defaultsellingmode-control-added]**
...
- No payment/checkout flow and no bazaar-recommendation logic designed anywhere in this document. `defaultSellingMode` was briefly in scope (`decision-log.md` D27) and is now retired at the Foundation level (`decision-log.md` D79) — nothing left in this document to design against it. **[see settings.changelog.md#decisions-defaultsellingmode-scope-no-longer-excluded]**
...
- **"Cómo vendes normalmente" retired in full, 2026-09-18 (`decision-log.md` D79) — the Product Owner's own explicit instruction: "I do not want two separate concepts... there should simply be an NFC capability setting."** `Business.defaultSellingMode` has zero remaining functional readers anywhere in the Foundation. §2.2's table drops to three actions across two capabilities; §2.3 is retired in full (not merely narrowed); §2.6's already-historical handoff mechanism is confirmed doubly moot; §2.8 becomes the sole NFC-related control, unconditionally, for every Paid-tier Business; §3.3a/§3.6 collapse to one wireframe shape each, with no `buttons`-mode/`nfc`-mode distinction of any kind. **[see settings.changelog.md#decisions-d79-como-vendes-normalmente-retired]**
...
- Whether `home.md`'s own "Leer con NFC" overlay (the consequence §2.8 discloses) needs any *ambient* signal beyond this document's own confirmation copy — not designed here, `home.md`'s own scope (§8, item 19).
