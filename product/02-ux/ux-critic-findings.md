...
- **Flagged for Main, not a finding against `events.md`/`reports.md`:** `home.md` was outside this update's scope and still renders the compound string "Bazar Plaza Norte" in its session-header wireframes (§3.5–§3.10, §3.13) — the same Type+Place-in-one-field composition `events.md` §10 names as the anti-pattern Venue exists to retire. May be a deliberate space-constrained compression for Home's single-line header, but it's currently undocumented as intentional rather than stale. Not blocking this cycle; worth a short follow-up look at `home.md` at some point, tracked here rather than actioned now.
