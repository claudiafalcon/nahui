...
## Live identity preview, "Tu negocio" (`frontend-design` audit proposal #4, 2026-08-15) — `onboarding.md` §3.9/§3.9a/§3.9b

`onboarding.md` §3.9/§3.9a's wireframes gained a new, non-interactive live preview element — a small, explicitly-labeled card reflecting `Business.name`/`Business.logo` as she types/selects, reusing the visual prominence `home.md` §3.8f already gives them on the real Digital Receipt. New shared subsection §3.9b defines its five screen states, placement reasoning, accessibility approach, and visual differentiation from the real receipt. Sourced from `product/02c-high-fidelity-prototype/docs/design-audit-2026-08-15.md` proposal #4; grounded via a `knowledge-mentor` consultation (no existing precedent in this document family for a live/as-you-type field-reflection pattern).

### ONB-PREV-MAJ1, closed

Placement reasoning (below the fold, immediately above "Continuar") never established that the preview would actually be seen in the dominant no-logo fast path — the exact case §2.2b names as the expected common one, and §6's own tap-floor table confirms as the fastest real path.
Status: Fixed. Four new paragraphs added to §3.9b's Placement section: a reasoned confirmation that "Continuar" sits below the fold even in the minimal no-logo rendering (every field's affordance renders unconditionally regardless of what's filled in, so screen height is governed by the full field set, not by what she interacts with); an alternative placement (anchoring below Nombre) explicitly considered and rejected (reintroduces the same keyboard-competition risk a different way); a residual, not-fully-closeable-by-layout-reasoning risk (does she register a passive box during a fast, uninterrupted scroll) logged as new §8 item 8 rather than silently asserted solved; an acknowledgment of the screen's stacked explanatory-text density.

### ONB-PREV-MIN1, closed

The logo-only substitution (state 3 — logo replaces Nombre-as-text in the preview box, matching `home.md` §3.8f's real-receipt rule) wasn't called out on screen when it happened — the box's content changed with no marker distinguishing intentional substitution from her name silently dropping out.
Status: Fixed. State 3 gained its own persistent (not transient) on-screen caption — "Tu nombre sigue guardado arriba. En tu recibo se muestra tu logo." — present for as long as state 3 is showing.
...
## Concurrent multi-seller selling + Event-scoped inventory allocation (`product-decisions.md` Q24/Q25, 2026-09-06/07) — `authentication.md`, `settings.md`, `home.md`, `events.md`

**Retroactive entry — added 2026-09-09, closing a documentation gap `reviewer` flagged (this log had no entry for this batch despite the cycle being complete).** The full four-document design (Invitation acceptance/decline flow, "Tu equipo" staff management, the SELLER role experience including multi-Event resolution and the "lost the race" concurrent-selling conflict pattern, and the full `EventAllocation` lifecycle) is not independently re-narrated here — the authoritative record is each document's own status header (all four dated 2026-09-06/07) and `product/02-ux/product-decisions.md`'s Q24/Q25 entry. Summary: `ux-critic`'s first full-batch pass found 4 Major + 4 Minor, 0 Blockers (a CTA/heading collision, an undefined lost-race outcome discovered by Finalizar Venta's own write, a false Registrar-Mercancía-precedent claim in `events.md` §3.21 corrected to a genuine collapse-to-summary redesign, a wireframe/prose contradiction), all fixed; the §3.21 redesign produced one self-caught regression in `events.md` §6's task-efficiency table (found and fixed across two further narrow verification passes). `reviewer`'s pass found 0 Blockers, 3 Important documentation-persistence gaps (a stale `product-decisions.md` status paragraph, `BusinessMembership.status`/`revokedAt` missing its own `decision-log.md` entry — closed as D55 — and a business-decisions.md numbering collision) — all closed directly by Main. All four documents folded back into Approved, 2026-09-07.

### Verification

`ux-critic` re-verification: all 8 findings confirmed resolved, no regressions, per each document's own status header. `reviewer`: clean on final pass. See `product/02-ux/product-decisions.md` Q24/Q25 for the full narrative and `company/bitacora.md` (2026-09-06/07 entries) for the project-log record.

## `EventAssignment` — assignment-scoped SELLER Event picker (`product/99-rfc/0011-event-assignment.md`, 2026-09-09) — `home.md` §2/§3.3/§3.4/§3.5/§3.6b

RFC 0011 (Proposed, Product-Owner-confirmed design-settled) narrows a SELLER's Session-open Event-resolution to only Events she holds an `EventAssignment` for, adding a new passive awareness line for the newly-reachable zero-assignment-with-Events-active-elsewhere case.

### Findings, round 1 — 1 Major + 2 Minor, all fixed

- **HOME-EA-MAJ1 — Fixed, verified.** The new SELLER passive line ("Hay eventos activos hoy, pero no estás asignada a ninguno.") ended on the bare exclusion fact, with no reassurance/resolving clause — breaking this document's own established state-then-resolve shape (every comparable precedent, §3.6a's Not Ready/capability-revoked mentions and §3.17's "Acceso no disponible," closes on either a reassurance clause or a real next step). Worst on §3.3's cold-start combination (two unresolved negative facts, zero CTA, zero positive element on the whole screen). Fixed: copy revised to "Hay eventos activos hoy y no estás asignada a ninguno. Pídele a quien te invitó que te asigne a uno." — closing on the same resolving-clause shape its own SELLER siblings already use, dropping the "pero" construction none of the cited precedents use.
- **HOME-EA-MIN1 — Fixed, verified.** §3.5's stacking order for up to three co-occurring ambient elements (upcoming-Event card, same-day-resume line, the new SELLER line) was left unspecified, unlike §3.4/§3.6's explicit ordering for their own cases. Fixed: explicit order stated (card → same-day-resume → CTA → SELLER line → §3.6a line).
- **HOME-EA-MIN2 — Fixed, verified.** The new CTA-sandwiched-between-two-ambient-lines composition at §3.4/§3.5 lacked the High-Fidelity legibility flag this document uses elsewhere for comparable risk (§3.7/§3.9's precedent). Fixed: equivalent flag added at §3.4, cross-referenced from §3.5.

### Verification

`ux-critic` re-verification pass (2026-09-09): all three findings confirmed closed, no regressions, no stray old copy (one old-copy occurrence at §3.4 is a deliberate historical quote inside the design note explaining the fix, not a leftover). `reviewer`'s Foundation-consistency pass: 0 Blockers, 0 content defects; 3 Important documentation-persistence findings (this log's own missing entries for both this round and the batch above; a stale `home.md` status-header closing line; `product-decisions.md`'s Q24/Q25 entry not reflecting RFC 0011 Open Item 1's Resolved status) — all closed directly by Main. Folded back into Approved.

## Manual-mode reconciliation — inventory-commitment lifecycle correction (`product/99-rfc/0010-event-scoped-inventory-allocation-commitment-lifecycle-correction.md`, 2026-09-09) — `events.md` §3.16/§3.24

RFC 0010 (Proposed, Product-Owner-confirmed design-settled) corrects RFC 0009's manual-mode allocation mechanism (a confirmed defect: a disconnected counter that never actually removed committed stock from the shared pool) and, per explicit Product Owner direction, redesigns Event-close reconciliation so neither mode ever auto-releases silently — both require an explicit merchant action, differing only in evidentiary shape (quantity-confirmed for manual, unit-identified for NFC, unchanged).

### Findings, round 1 — 1 Major + 2 Minor, all fixed

- **EVT-RFC0010-MAJ1 — Fixed, verified.** A mixed row (both an NFC sub-pool and a manual sub-pool outstanding on the same Product) showed two identical, undifferentiated "Mover a otro evento" buttons with no visual grouping between the two pools — a real mis-tap risk, since the two buttons move genuinely different stock, and §3.24's confirmation screen didn't name which pool was being moved so a mis-tap wasn't even catchable one screen later. Fixed: a blank-line grouping matching §3.21's own expanded-row precedent separates the two sub-blocks, and each button carries a mode qualifier ("de las con tag" / "de las sin tag") — present only on a mixed row; a single-pool row keeps the plain unqualified button.
- **EVT-RFC0010-MIN1 — Fixed, verified.** Tapping "Mover a otro evento" while "Ajustar cantidad"'s stepper was mid-reveal had no defined effect on the staged, unconfirmed stepper value. Fixed: states explicitly that the staged value is silently discarded, identical to Cancelar, matching §3.21's own "leaving before the commit action discards unsaved edits" precedent — applied consistently in both §3.16's prose and §4's flow mirror.
- **EVT-RFC0010-MIN2 — Fixed, verified.** The ambient checkmark on a confirmed-shortfall/zero reconciliation outcome diverged from this document's own §3.13 "Evento cancelado" no-checkmark precedent for neutral/negative outcomes. Fixed: checkmark dropped on any confirm below the full expected amount (N=1's "No regresó," and the stepper path whenever the confirmed value is less than N) — kept only for a genuine full return.

### Verification

`ux-critic` re-verification pass (2026-09-09): all three findings confirmed closed, no regressions, no stray reference to the old unqualified/undefined/checkmark behavior anywhere in the document (traced every `✓`/`regresaron`/`regresada` occurrence). `reviewer`'s Foundation-consistency pass: 0 Blockers, 0 content defects — every RFC 0010 term (`unitSource`, `quantityExpected`, `releaseAllocation()`, the `type=return_to_general` reuse) verified accurate against the RFC's current (reworked) text, no redesign of anything RFC 0010 itself already settled. 5 Important documentation-persistence findings (this log's own missing entry — this section; `product-decisions.md`'s stale "settled architecture" description of the now-corrected counter mechanism; `product/99-rfc/README.md`'s stale RFC 0010 summary line; `company/bitacora.md`'s same-day entry describing the pre-rework design; a forward-tracking gap — `home.md` §3.8a-d/§3.9 still describe the counter mechanism this RFC retires, now logged as RFC 0010's own Open Item 7) — all closed directly by Main. 2 Suggestions (a "settled" vs. "design-settled" wording precision; an incomplete idempotency-bullet enumeration) — both applied. Folded back into Approved.

## Personal para este evento — OWNER-side staff-to-Event assignment (`product/99-rfc/0011-event-assignment.md`, 2026-09-09/10) — `events.md` §3.11/§3.14/§3.15/§3.26

RFC 0011 (Accepted, D60) named the SELLER-side consumption (`home.md` §2/§3.6b, reviewed separately above) but left the OWNER-side surface — actually creating/managing `EventAssignment` rows — entirely undesigned anywhere. `events.md` §3.26 "Personal para este evento" designs it: one row per active SELLER `BusinessMembership`, grouped Asignadas/Sin asignar, single-tap assign/unassign, a live scheduling-conflict warning (warn-with-override, per RFC 0011's own settled rule).

### Findings, round 1 — 3 Major + 1 Minor, all fixed

- **EVT-RFC0011-MAJ1 (originally logged as EVT-M4) — Fixed, verified.** The scheduling-conflict warning was shown only *after* the "Asignar" tap completed, degrading the informed-choice moment the whole RFC exists to support — an OWNER comparing several unassigned candidates had no way to see who was already committed elsewhere without an assign/read/unassign cycle per candidate. Compared unfavorably against `home.md` §3.6a's Limited Ready recommendation, shown pre-tap for exactly this reason (a prior defect there, HOME2-MAJ2, was fixed specifically to keep that signal pre-commit). Fixed: the conflict line now renders on every row — Asignada or Sin asignar — computed live on every render, before any tap. The write itself still never blocks.
- **EVT-RFC0011-MAJ2 (originally logged as EVT-M5) — Fixed, verified.** The zero-state conflated "never invited anyone" with "invited someone, still awaiting acceptance" — a merchant who followed the screen's own "invita a alguien... luego regresa aquí" instruction landed back on the identical "todavía no tienes a nadie" copy. Fixed: a second, distinct zero-state now checks for a pending `Invitation` and shows honest, non-repetitive copy for that case.
- **EVT-RFC0011-MAJ3 (originally logged as EVT-M6) — Fixed, verified.** The repeated "structurally absent from Free-tier Business" claim overstated what was actually settled — traced to `settings.md` §8 item 13, then a genuinely open item. `architect` resolved it directly from existing Foundation precedent (D55/D56, D25, D27, D40): an already-active SELLER Membership survives a Paid→Free downgrade unaffected; only *creating a new* `Invitation` is Paid-gated. Fixed: "Asignar personal"/"Ver personal de este evento" are now present unconditionally regardless of `subscriptionTier` (corrected in five places); `settings.md` §8 item 13's own text corrected to match (its original parenthetical had incorrectly asserted "Asignar personal... correctly still Paid-gated," contradicting the resolution it was meant to record).
- **EVT-RFC0011-MIN1 (originally logged as EVT-MIN1 — renamed here to avoid colliding with this log's own pre-existing, still-Open `EVT-MIN1` for "Cancelar evento's write action has no loading/error state," an unrelated finding) — Fixed, verified.** Zero-tap Asignar/Quitar on phone-number-only rows carried a narrow, fully-recoverable mis-tap risk. Fixed: a light, optional row-settle visual mitigation added on success — doesn't add a tap or a confirmation step.

### Verification

`ux-critic` re-verification pass (2026-09-10): all four findings confirmed closed, no regressions — traced every `subscriptionTier`/`Free tier`/`Paid tier` occurrence in the document and confirmed no stale claim survives anywhere. `reviewer`'s Foundation-consistency pass: 0 Blockers — confirmed the `BusinessMembership.status=active`-only gate (no `subscriptionTier` condition) against RFC 0011's/D60's actual text, confirmed `settings.md` §8 item 13 and `events.md` no longer contradict each other, confirmed the D17/D53 scheduling-conflict citation is correctly never conflated with `EventAllocation`'s own physical-location-exclusivity invariant (two different kinds of "conflict" in this Foundation), confirmed `ubiquitous-language.md`'s `EventAssignment` entry needs no correction. 4 Important documentation-persistence findings (this log's own missing entry for this round — this section, closing the same EVT-MIN1 ID collision named above; `product/99-rfc/README.md`'s stale "Proposed" status line for RFC 0011; `product-decisions.md`'s stale "(Proposed)" tag; `decision-log.md` D60's own "Applied:" line omitting `events.md` entirely) — all closed directly by Main. Folded back into Approved.

## Tu equipo — token-based Invitation rework (`product/99-rfc/0013-invitation-token-based.md`, Accepted, `decision-log.md` D64, 2026-09-13) — `settings.md` §2.7/§2.7a/§3.11–§3.14

RFC 0013 reworked `Invitation` from phone-keyed to token-keyed (a secure, single-use shareable link replaces requiring a phone number to invite a SELLER), the direct consequence of D63 activating Google/Email as phone-free cold-sign-up methods — it became inconsistent for an OWNER to still need a SELLER's phone number to invite her once phone stopped being required to become a User at all. `settings.md` §2.7/§3.11–§3.14 ("Tu equipo") reworked to match: link generation replaces phone entry, an optional `targetHint` email rides along as a pure memory aid, and §8 items 11/12 (cancelling a pending Invitation; `expired`'s undefined trigger) were bundled and closed in the same pass per RFC 0013's own suggestion.

### Findings, round 1 — 3 Major + 2 Minor + 2 specialist-consultation items, all closed

- **SET-RFC0013-MAJ1 — Fixed, verified.** §3.12c ("Invitación lista") never stated what happens if the app is backgrounded or reloaded before she taps Copiar/Compartir/Listo — a real risk given `company/CLAUDE.md`'s own Core Thesis names unpredictable customer-flow interruption as Ana's #1 validated friction, and RFC 0013 §4 makes the plaintext link genuinely unrecoverable after this one render (tokens are stored hashed). Fixed: the limitation is now stated plainly as a named, accepted constraint rather than left silent.
- **SET-RFC0013-MAJ2 — Fixed, verified.** §4's "Generar otra" branch (regenerating an expired Invitation) never named its own destination — RFC 0013 itself left this write path unspecified (mutate the existing Invitation in place vs. create an independent second one), and the ambiguity risked her seeing a confusing orphaned "Invitación caducada" row sitting next to an unrelated new one. Fixed: resolved explicitly as an in-place mutation (same `id`, fresh `token`/`status`/`expiresAt`) — the choice that actually makes "regenerate" read as fixing the offer, not creating a duplicate.
- **SET-RFC0013-MAJ3 — Fixed, verified, via `knowledge-mentor` consultation.** §3.12c's "Listo" was left ungated on having copied/shared the link first, defended only by brand-tone reasoning ("respects the vendor's intelligence") — a usability-safety question `ux-critic` correctly named as unsettled by tone alone, since no prior screen in this family had shown a one-time, non-recoverable secret to a non-technical, interruption-prone user before. `knowledge-mentor` confirmed general practice (password managers, 2FA backup codes, API-key flows) uniformly gates the dismiss action for exactly this shape of risk, grounded in Norman's forcing-function principle and Nielsen's error-prevention-over-warning heuristic — and that this project already holds itself to the identical standard elsewhere (`settings.md §3.14`'s own earlier fix from a silent to a positively-confirmed exit). Fixed: "Listo" now disables until Copiar enlace or Compartir has fired at least once; step-count table (§6) corrected to reflect the real minimum (4 taps, was undercounted at 2).
- **SET-RFC0013-MIN1 — Fixed, verified.** §3.11 never explained why `Invitation.status = accepted` has no row of its own, unlike every other state transition this document narrates carefully. Fixed: one sentence added (absorbed into the new active `BusinessMembership` row at accept-time, an atomic write per RFC 0013 §2 step 6).
- **SET-RFC0013-MIN2 — Fixed, verified.** §3.12's helper text used completed-action phrasing ("a quién se la mandaste") on a form shown before anything is sent. Fixed: reworded to avoid the tense mismatch entirely ("a quién es esta invitación").
- **`brand-guardian` consultation — Fixed, verified.** §3.12c's risk-disclosure sentence ("Cualquiera que lo abra puede aceptar...") was flagged as new tonal territory (the first line in this document family disclosing a property of the mechanism itself, not a consequence of Ana's own choice) rather than assumed covered by existing precedent. Finding: Minor — the underlying fact was correct and necessary, but the phrasing (an elided-object fragment, a generic "cualquiera" subject) read closer to a disclaimer label than Nahui's own voice, disconnected from §3.12's own "alguien de tu confianza" framing one screen earlier. Fixed: reworded to "El enlace funciona para cualquiera que lo abra — compártelo solo con la persona de tu confianza," keeping the fact exactly as blunt while reconnecting to the established "confianza" vocabulary.

### Verification

`reviewer`'s Foundation-consistency pass (2026-09-13): 0 Blockers. Every `Invitation.token`/`expiresAt`/`targetHint`/`acceptedByUserId` schema claim confirmed accurate against `domain-model.md`/`ubiquitous-language.md`'s actual current text (both correctly updated under D64). The "Generar otra mutates in place" write-semantics resolution confirmed as not contradicting any Foundation invariant — RFC 0013 genuinely left it open, and an in-place mutation of already-modeled fields crosses no aggregate boundary. The "Listo" gating change confirmed as pure client-side UI state needing no Foundation representation. 3 Important documentation-persistence findings — this log's own missing entry for this batch (this section, closing it); `product-decisions.md`'s Q24/Q25 "settled architecture" bullet still describing the retired phone-keyed `Invitation` schema with no forward pointer; `authentication.md` §8 item 9 and §11 both carrying the identical stale "unresolved there" claim about `settings.md §8` items 11/12 that `settings.md`'s own audit had only caught at one of its three occurrences (the inline §2.2a parenthetical) — all closed directly by Main. 1 Suggestion (the "generar otra" phrase used for two different operations in §3.11 vs. §3.12 — reworded to "crear otra invitación" in the duplicate-`targetHint` advisory, matching this document's own "Invitar a alguien" vs. "Nueva invitación" disambiguation precedent) — applied. Folded back into Approved.
