...
**Amended 2026-09-04 (`decision-log.md` D52) — this route is now build-gated; the paragraph above's central premise no longer holds.** The Product Owner asked for Acceso DM to be moved into the existing, isolated demo-campaign build (`VITE_DEMO_MODE=true`, `demo.nahui.app`) instead of shipping inside the one real production bundle — structurally absent from production, the same bar `demo-mode.md §1` already holds itself to (for a different reason: campaign-scoped content, not curated-audience reachability). The paragraph above's load-bearing claim — "it does ship in the one real production build," the fact that put this document on D13/D45's reachability clause rather than the build-artifact clause — is no longer true. `D48`'s premise ("no code path at all in a real, shipped build") now applies to this route directly, exactly as it already applies to `demo-mode.md`; this supersedes the paragraph above's own reachability-clause reasoning on that one point (the mechanism/architecture findings elsewhere in this document — `commitLot` retry-safety, the `nahui-acceso-dm-active` flag, the results-guidance nudge — are unaffected and still describe how this route actually works). Mechanically: `DemoModeGate.tsx`'s real-build branch is a bare `<AppRouter />` with no import path to `AccesoDmGate.tsx` (or anything else under `src/screens/AccesoDM/`) reachable from it at all; the `?acceso=dm` marker check now runs only inside the demo-campaign branch, ahead of `DemoModeGateActive`'s own resolution, so a fresh device still lands on the seeded catalog with zero taps — never seeing `demo-mode.md`'s own Welcome screen. See `decision-log.md` D52 for the full record, including why D51 stays (marked superseded on this one point, not deleted) and the corrected naming rationale below.
...
**Retired 2026-09-10 (Product Owner decision, `decision-log.md` D61 — superseding D51/D52 on this point).** This route (`?acceso=dm`) is retired in full, together with the demo-campaign build it lives inside as of D52 (`demo.nahui.app`, `demo-mode.md`) — now that real WhatsApp OTP is implemented, that build's own authentication flow mirrors `nahui.app`'s, closing the low-friction, no-real-phone-number gap this route existed to bridge for DM-qualified merchants. Going forward, a DM-qualified merchant reaches the real product the same way every other merchant does: `nahui.app` → WhatsApp OTP → real business → real Nahui usage — no separate auth-free shortcut. This document's own content below (mechanism, wireframes, decision logic, analytics instrumentation) stays in place, unedited, as historical record, per this project's non-deletion discipline. Not a correction of D51's mechanism/architecture findings or D52's relocation record — both were sound rulings for a route that served a real purpose at the time; that purpose no longer exists, not because either ruling was wrong.
...
3. **Resolved, 2026-09-04 — both premises below are now stale (`reviewer` finding during the D52 relocation review).** Both parts of the original gap are closed: (a) `company/merchant-validation-concierge-pilot.md`'s Loop-2 handoff was wired to send this route's link (commit `ee1e845`), then corrected to `https://demo.nahui.app/?acceso=dm` once D52 moved this route into the demo-campaign build; (b) the "no instrumentation as designed" cost named below no longer applies either way — post-D52, this route automatically inherits `demo-mode.md §2.5`'s entire `demo_*` suite as a side effect of now compositing `ReminderBanner` (accepted, not chosen — see D52's own "known, accepted side effect" note; it conflates DM-pilot traffic with ordinary self-serve visitors in that specific event stream), in addition to this route's own dedicated `acceso_dm_*` suite (§2.4), which stays correctly DM-pilot-only. Kept below, unedited, as the original reasoning that was live when the pilot launch decision was actually made — not deleted per this project's non-deletion discipline.
   - **Further corrected 2026-09-04 — the `demo_*`/`acceso_dm_*` conflation named just above is now resolved, not merely accepted.** `ReminderBanner.tsx` is gated on `isAccesoDmActive()` at the component level: an Acceso DM session never mounts the component that fires `demo_*` events or renders the questionnaire-Form banner content, mounting instead a lean pass-through that renders only this document's own `ResultsGuidanceNudge`. An Acceso DM session now produces only `acceso_dm_*` events; `demo.nahui.app`'s ordinary self-serve visitors are unaffected. See `decision-log.md` D52's own dated correction note for the full record.
