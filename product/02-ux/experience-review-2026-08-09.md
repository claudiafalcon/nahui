...
**Verification status: Tooling Artifact.** `ui-designer` inspected ground-truth `node.reactions` data (not click-testing) across the full subgraph both walks reported as going dead — `162:1485`, its "▾"/"Registrar mercancía" triggers, the Configuración sheet and its destination, the NavBar tabs, and all 33 legitimate callers of the "Registrar mercancía" destination. Every reaction checked is correctly wired, well-formed, and unrelated to anything touched by the D40 deletions. This is the same symptom signature `infrastructure-decisions.md` ID012/ID013/ID014 already documented and root-caused twice before: a long-lived `chrome-devtools-mcp` page can stop delivering clicks on specific frames without any error surfacing, purely a browser-automation artifact, not a file defect. `merchant-user-tester` has no `press_key` tool, so it cannot self-apply the known workaround (click, then Enter) and will report this as a permanent dead-end regardless of how many times it's re-run on a long session. Not closed by inference alone, though — see below.
...
Both sessions ended with Ana stuck on the Home screen before ever reaching Resultados. The newly-synced Recompensas sub-chain (`798:488`–`798:589`) remains completely un-walked by any persona, since the tooling artifact above blocked both attempts before either got there. A third walk is the actual closing step now that the wiring itself is confirmed clean — routed with a more direct path to Resultados to reduce the chance of hitting the same artifact before reaching the un-walked content.
