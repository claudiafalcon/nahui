...
**Amended 2026-08-19/20 (fourth amendment this date — two more events, verified against `home.md`/`settings.md`, designed, reviewed, built, live-verified).** Two further events extend §2.5.2's already-built three: `demo_sale_completed` (fires once per finalized Sale, repeatable within a session — `home.md`'s selling flow) and `demo_paid_plan_activated_midsession` (fires when `subscriptionTier` transitions to `'paid'` via Configuración's "Activar plan de pago," §2.2/§3.4, distinct from `demo_onboarding_completed`'s `path` property, which reflects the tier chosen *at* onboarding, not a later self-initiated upgrade). `ux-designer` verified both trigger definitions directly against their owning Approved specs and the built code (`finalizeSale()`, `activatePaidPlan()`). For `demo_sale_completed`, no edge case (cancelled sales, sale reopening, partial finalization) miscounts or is missed. **Correction (2026-08-19, `reviewer`):** for `demo_paid_plan_activated_midsession`, this verification was not, in fact, clean — a real edge case was found, not absent. The originally-specified "saw non-paid → fired" guard, if armed on `businessForCurrentUser(state)` returning `null`, false-fires the instant Onboarding completes on both the Paid and "Ver un ejemplo" paths, since `completeOnboarding()` (`store.tsx:521-558`) writes `subscriptionTier: 'paid'` directly on those two paths with no intermediate free-tier Business ever existing — a routine occurrence, not a rare one. Found and fixed the same day; see §2.5.3's corrected trigger condition below. Same shape as §2.5.2: count-only, external observers of shared `AppState` via `useStore()`, zero Merchant Application component edits, `ReminderBanner.tsx` remains the sole mount point. See new §2.5.3.

**Amended 2026-08-19 (third amendment this date — documentation gap, `reviewer`-flagged during review of a pending 3-event instrumentation design; retroactive + prospective).** This document has never mentioned analytics, `track()`, or `@vercel/analytics` anywhere in its own text, despite one event — `demo_questionnaire_cta_click` — already live in production since the banner build that shipped this same day's earlier "Modo demo" label amendment above. `reviewer`, reviewing a new three-event instrumentation design (`demo_pass_through_reached`, `demo_otp_completed`, `demo_onboarding_completed` — not yet authorized to build, see below) against this document, flagged the gap explicitly and recommended this amendment retroactively document the already-live event too, rather than landing three more undocumented events on top of an already-undocumented one. Three things happen here: **(1)** `demo_questionnaire_cta_click` is documented for the first time, retroactively, exactly as built (new §2.5.1); **(2)** §1's Acceptance Criteria bullet claiming "no analytics/pixel installed" (citing `company/merchant-validation-campaign-meta-ads.md` §7) is corrected — stale since that same banner build mounted `@vercel/analytics`; **(3)** the three-event design — already reasoned through and reviewed by `ux-designer` and `reviewer` in a design/review pass, not decided here — is documented as designed-and-reviewed-but-not-yet-authorized-to-build (new §2.5.2), pending one further prerequisite: `demo_questionnaire_cta_click`'s own end-to-end delivery to the Vercel Analytics dashboard is currently unconfirmed (`company/merchant-validation-funnel-diagnosis.md` Stage 8) — it queues correctly client-side per direct live testing, but was never observed to actually transmit as a network request, and two candidate causes (Vercel plan tier possibly lacking Custom Events; automation/webdriver filtering on the test session) remain undistinguished. Since all three new events would share the identical delivery mechanism, Main is holding this design from implementation until that's resolved.

**Status: documentation gap closed, same day. No `ux-critic` dispatch needed** — none of the three pieces above changes any wireframe, screen state, or participant-facing copy; every event described is a silent, client-side count with zero UI surface, the same reasoning already applied to Architecture Review items 6/7 (§8) folding directly into `reviewer`'s own pass rather than a separate dispatch. **No `brand-guardian` consultation needed** — event names (`demo_questionnaire_cta_click` and the three new identifiers) are internal/agent-facing labels, never rendered in participant-facing copy; nothing here is a new tonal or emotional touchpoint. `reviewer`: the finding that triggered this amendment; re-verification of this finished text pending. **This section is not "Approved" in the sense the rest of this document uses that word for participant-facing design** — it is a documentation-completeness fix (items 1-2) plus a design record of already-reviewed, **not-yet-build-authorized** instrumentation (item 3). See new §2.5, the corrected §1 bullet, §8 items 9-10, §9, and §10 below.
...
**Retired 2026-09-10 (Product Owner decision, `decision-log.md` D61 — superseding D48).** The separate demo-campaign build this entire document specifies (`demo.nahui.app`, `VITE_DEMO_MODE=true`) is retired in full: now that real WhatsApp OTP is implemented, this build's own authentication flow mirrors `nahui.app`'s, so it no longer offers the low-friction, no-real-phone-number experience it was built for — asking someone to complete real WhatsApp OTP just to enter a disposable, throwaway demo has no remaining product value. Going forward: interest/DM/organic discovery → `nahui.app` → WhatsApp OTP → real business → real Nahui usage. No separate demo deployment or auth path. This document's own content below (Welcome screen, persistent Form-reminder banner, restart mechanism, analytics instrumentation) stays in place, unedited, as historical record, per this project's non-deletion discipline — none of it is removed, only this document's status changes. Sample/example functionality may remain or evolve as a capability inside the real product later — see `onboarding.md`'s own, unaffected "Ver un ejemplo" path, which is not part of this retirement.
...
**Business objective — analytics instrumentation (added 2026-08-19, retroactive + prospective).** Independent of what any single touchpoint conveys to a participant, give Main/Marketing a way to measure funnel progression through this document's own screens — `company/merchant-validation-funnel-diagnosis.md` found the largest unmeasured share of the 423-views/3-responses acquisition gap sits precisely in the stages this document's own flow covers (Stages 4-7: prototype start, OTP completion, Onboarding completion, reaching the banner). One event (`demo_questionnaire_cta_click`, §2.5.1) is already live; three more (§2.5.2) are designed and reviewed, and, as of later the same day, build-authorized — §8 item 10's delivery-confirmation prerequisite is satisfied (Product Owner confirmed real counted data in the Vercel Analytics Events panel).
...
- The reminder cannot detect whether she's already submitted the Form — it keeps showing regardless. Re-opening an already-completed public Form is harmless, not a defect. **Corrected 2026-08-19:** this bullet previously cited "no analytics/pixel installed" (`company/merchant-validation-campaign-meta-ads.md` §7) as the accepted limitation underlying this — no longer accurate. `@vercel/analytics` is mounted and firing custom client-side events as of this document's own §2.5; what remains true, and is the actual accepted limitation here, is that no backend, pixel, or server-side logging exists anywhere in this project (`company/merchant-validation-funnel-diagnosis.md` §1's Root Cause framing, confirmed against the repo: no `api/` directory, no `vercel.json`). Client-side, count-only analytics cannot read or infer Form-submission state either way, since Google Forms is a separate, unrelated platform with no webhook or callback this app could observe. The underlying limitation is unchanged; only the "no analytics at all" framing that used to explain it was stale.
- **(Added 2026-08-19)** Analytics events, where they exist, are count-only or a single closed-enum-property-plus-count, zero PII, and never require any Merchant Application component to be edited to add them — see §2.5.
...
- **(Added 2026-08-19)** No server-side, pixel, or Meta Pixel logging — analytics stay strictly client-side, count-only, via `@vercel/analytics` (§2.5). This was already true before this amendment; only the §1 bullet's stale "no analytics at all" framing needed correcting (see corrected bullet above).
...
### 2.5 Analytics instrumentation (added 2026-08-19)

Not a screen-resolution branch like §2.1-§2.4 above — no new screen state, no new §5 entry needed (analytics events observe existing states/transitions; they never constitute one themselves). Documented here because, like §2.1-§2.4, this is decision logic: precisely when something fires, on what condition, with what payload.

#### 2.5.1 `demo_questionnaire_cta_click` — live in production (retroactive documentation, added 2026-08-19)

Already shipped, in the same build that introduced the "Modo demo" row-2 label (2026-08-19 amendment above) — this subsection documents it for the first time, exactly as built, closing the gap `reviewer` flagged.

- **Fires:** `track('demo_questionnaire_cta_click')` from `@vercel/analytics`, called inside `ReminderBanner.tsx`'s existing row-1 tap handler — the same handler that already calls `window.open('https://forms.gle/ZZhtJEfee3viWY1h8', '_blank')` (§2.3 check 3, §4). One call added alongside an existing one; no new interactive zone, no new tap target — §2.3 checks 3/4 and §4's "two independent interactive zones" line are all unaffected by this documentation, since nothing about the banner's tappable surface changed to add it.
- **Payload:** count-only. No properties, no PII, no phone number, no session identifier — a bare event name, nothing else.
- **Purpose:** splits "reached the CTA and declined to tap it" from "never reached it at all" for the acquisition funnel — this is Stage 8's own client-side signal, cited there by name (`company/merchant-validation-funnel-diagnosis.md` Stage 8).
- **Delivery status: CONFIRMED (updated 2026-08-19, later the same day).** Product Owner checked the live Vercel Analytics dashboard directly (Analytics → Web Analytics → Events panel) and confirmed `demo_questionnaire_cta_click` is present and counting — 2 visitors / 2 total events, real production data. This resolves the prior "queues but unconfirmed transmission" finding: the delivery mechanism works end-to-end. Of the two candidate blockers previously named (Vercel plan tier lacking Custom Events; webdriver/automation filtering on the test session), neither is blocking in practice — the Events panel simply wasn't scrolled into view during the earlier check. §8 item 10's prerequisite is satisfied; §2.5.2's build gate is now open.

#### 2.5.2 Three new events — designed, reviewed, **built (2026-08-19)**

**Status: built (2026-08-19).** `ux-designer` designed all three; `ui-designer` built all three into `ReminderBanner.tsx` (confirmed the sole mount point — no Merchant Application component edited); `reviewer` independently verified the built code against this spec (trigger conditions and their mount-time guards, payload shape, and the `pathFromCapabilities` reuse) and found it sound. Not yet independently delivery-confirmed against the live Vercel Analytics dashboard the way `demo_questionnaire_cta_click` (§2.5.1) was — that check should follow once this build is deployed, before this status is upgraded to "live in production."

All three share the same shape: count-only, or count-plus-one-derived-capability-enum; zero PII; external observers of shared `AppState` via `useStore()`. No Merchant Application component is edited to add any of them — `ReminderBanner.tsx` remains the sole mount point, the identical "sibling, never inside" discipline §8 item 2 already established for the banner itself.

**`demo_pass_through_reached`**
- Fires on first render of `ReminderBanner.tsx` — covers both routes into `pass-through` (§2.1): a fresh "Empezar demo" tap (§2.2), and the silent skip for an already-acknowledged device (§2.1 check 2). Both resolve to the same banner mount, so one event covers both without distinguishing which route reached it — that distinction isn't needed for the hypothesis this event feeds (`company/merchant-validation-funnel-diagnosis.md` Stage 7: whether the banner actually rendered, not which route reached it).
- Payload: none. Count-only.

**`demo_otp_completed`**
- Fires when `state.currentUser?.phoneVerifiedAt` transitions from null/absent to a set value, for the first time in the session — read via `useStore()`, an external observer; no edit to `CodeStep.tsx` or `AuthenticationFlow.tsx`. Feeds `company/merchant-validation-funnel-diagnosis.md` Stage 5.
- Payload: none. Count-only.

**`demo_onboarding_completed`**
- Fires when `business.onboardingAcknowledged` transitions from false/absent to `true` — the `acknowledgeOnboarding()` write, `OnboardingFlow.tsx:106`'s `TodoListo` `onEnter`. External observer, no edit to `OnboardingFlow.tsx`/`TodoListo.tsx`. Feeds `company/merchant-validation-funnel-diagnosis.md` Stage 6.
- Payload: `{ path: 'free' | 'paid' | 'demo' }` — derived via the existing `pathFromCapabilities` domain utility (`src/domain/onboardingResolution.ts`), not a new capability read invented for this event. The one property carried by any of these three events, and it's a closed three-value enum already computed elsewhere in the codebase for an unrelated purpose — not a new PII surface.

#### 2.5.3 Two more new events — designed, reviewed, built, live-verified (added 2026-08-19, verified 2026-08-20)

Same shape as §2.5.2: count-only, external observers of shared `AppState` via `useStore()`, zero Merchant Application component edits, `ReminderBanner.tsx` remains the sole mount point. **Status: built in `ReminderBanner.tsx`, and Main independently confirmed live in the running dev build (`VITE_DEMO_MODE=true`)** — walked the exact false-fire scenario the design correction targeted (choosing "Ver un ejemplo" at Onboarding) and confirmed `demo_paid_plan_activated_midsession` correctly does not fire, while `demo_onboarding_completed` fires once with `path: "demo"`; separately confirmed `demo_sale_completed` fires once on finalizing a real Sale. All four session events (`demo_pass_through_reached`, `demo_otp_completed`, `demo_onboarding_completed`, `demo_sale_completed`) fired in the correct order with no duplicates or false positives across the full walkthrough.

**`demo_sale_completed`**
- Fires once per finalized Sale — an external observer of `state.sales`, watching for any `Sale.status` transitioning to `'finalized'` (`finalizeSale()`, `store.tsx:887-945`, backing `home.md §3.8`'s "Finalizar Venta"). Unlike the four existing events, this is not a one-shot session latch: a participant may complete several sales during a walkthrough, each a distinct, valid engagement data point. Implementation: a `Set` of already-counted Sale IDs, seeded at mount with whatever Sale IDs are already `'finalized'` at that moment — the same "not-already-true-at-mount" guard `demo_otp_completed`/`demo_onboarding_completed` already use, adapted from a one-shot boolean to a per-ID Set since this event is legitimately repeatable — firing once for each Sale ID newly observed crossing into `'finalized'` thereafter. Not scoped to Quick Session vs. Event-linked Session: `finalizeSale()` looks up the active Session by `status = 'active'` only, never `eventId`, and `home.md`'s own model draws no distinction between the two for what counts as "a sale finished" — the only Quick-vs-Event divergence near this flow (Price resolution, §2) has no bearing on this event's fire condition. A cancelled Sale (`home.md §3.8b`, "Cancelar venta actual") never reaches this trigger — it's removed from `state.sales` outright, never marked `'finalized'` — and no reopen/edit path exists anywhere in `home.md` or the domain model for a Sale once finalized, so no double-count or miscount risk exists on either edge.
- Payload: count-only. No properties, no PII.
- Purpose: a within-session engagement-depth signal, distinct from the acquisition-funnel-stage events in §2.5.2 — how many real selling actions a participant actually performs once inside the demo, not merely whether she reached a given screen.

**`demo_paid_plan_activated_midsession`**
- Fires when `subscriptionTier`, resolved via `businessForCurrentUser` (the same resolution `demo_onboarding_completed` already uses, not `state.business` directly), transitions to `'paid'` **after** the observer has independently confirmed, in this same mount, a resolved (non-null) Business whose `subscriptionTier !== 'paid'`. The gate is deliberately **not** armed by `businessForCurrentUser(state)` returning `null` — the pre-Onboarding state, when no Business exists yet — because `completeOnboarding()` (`store.tsx:521-558`) writes `subscriptionTier: 'paid'` directly, with no intermediate free-tier Business, for both the Paid and "Ver un ejemplo" (demo) onboarding paths; treating "no Business yet" the same as "saw free tier" would false-fire this event the instant either of those two paths completes Onboarding — a routine occurrence, not a rare edge case. Since a Business is only ever created already resolved to its onboarding-path tier — never created `'free'` for a Paid/demo path and later silently flipping — this restricts the observed "free → paid" transition to the one real case the event is meant to measure: a genuinely Free-tier merchant later self-upgrading via Settings' "Activar plan de pago," an external observer of `activatePaidPlan()`'s write (`store.tsx:967-982`), reachable only via `settings.md §2.2/§3.4`, which renders only on the Free-tier vista principal (`settings.md §3.3a`). Genuinely distinct from `demo_onboarding_completed`'s `path` property, which reflects the tier chosen *at* onboarding — now that this gate correctly excludes the onboarding-completion moment itself, that distinction holds in practice, not just in intent.
- Verified against `settings.md §2.2/§3.4`: the confirm-then-save sequence (§3.4's "Confirmar y activar" → §3.9's saving state → the write) means the observed domain transition only occurs after a real confirmation, never on the initial button tap alone, and — since no failure path exists for this specific action in the current build — never misses a real completion either.
- `subscriptionTier` is not strictly one-way in the domain model (`reconcilePendingSubscriptionTier()` can land a deferred downgrade back to `'free'`), but that landing is gated on a real ~30-day-out effective date (`requestDowngradeToFree()`) with no reachable same-session fast-forward — so a second activation within one walkthrough isn't practically reachable today. Implementation uses a repeatable "saw free (resolved, non-null Business, `subscriptionTier !== 'paid'`) → fired" toggle, reset after each observed paid transition, the same repeatable shape as `demo_sale_completed` — corrected 2026-08-19 (`reviewer`) to never arm on a null Business — so this remains correct without a further design pass if the downgrade timing model ever changes (`settings.md` Q11, still open). Also carries the standard not-already-true-at-mount guard, matching `demo_otp_completed`/`demo_onboarding_completed`, evaluated only once a resolved Business exists — a null Business at mount arms nothing.
- Payload: count-only. No properties, no PII.
- Purpose: a self-initiated-upgrade engagement signal, distinct from onboarding's own tier choice — how many participants explore Configuración deeply enough to activate the paid plan on their own, mid-walkthrough.
...
- Analytics event firing — resolved automatically from state transitions/taps, never a manual toggle or participant-visible action (§2.5).
...
9. **Resolved, documentation only (this amendment).** `demo_questionnaire_cta_click` is retroactively documented (§2.5.1) — no architecture question remains open about the event itself, since it was already built and shipped; what's open is its delivery confirmation, item 10 below.
10. **Resolved 2026-08-19 (later the same day).** Whether `@vercel/analytics` custom events actually reach the Vercel Analytics dashboard end-to-end was unconfirmed as of this item's original text. Product Owner confirmed directly against the live dashboard: `demo_questionnaire_cta_click` is present in the Events panel with real counted data (2 visitors / 2 total). §2.5.2's build gate is open — `ui-designer` may proceed.
11. **New, same low-risk shape as items 6/7/10 — no separate `architect` dispatch needed.** `demo_sale_completed`/`demo_paid_plan_activated_midsession` (§2.5.3) are pure external observers of `AppState` via `useStore()`, mounted at the identical `ReminderBanner.tsx` sole mount point already established for §2.5.2 — no new domain write, no new persistence key, no new coupling to any Merchant Application component. Fold into `reviewer`'s own Foundation-consistency pass.
...
- **(Added 2026-08-19) #4 (internal-only entities never leak into user-facing language)** — event identifiers (`demo_questionnaire_cta_click`, `demo_pass_through_reached`, `demo_otp_completed`, `demo_onboarding_completed`) are internal/agent-facing names, never rendered in participant-facing copy. No tension.
- **(Added 2026-08-19) #6 (dependency direction)** — every §2.5.2 event is a pure external observer of `AppState` via `useStore()`; none writes to, branches on, or derives new domain state from what it reads (`phoneVerifiedAt`, `onboardingAcknowledged` are read only to detect a transition, never interpreted or acted on). No dependency-direction violation, the same distinction already drawn for restart's own storage clear above.
- **(Added 2026-08-19) #7 (idempotent/keyed retries)** — not applicable to any of the four events in §2.5: none performs a domain write; all are read-and-report only. `demo_questionnaire_cta_click`'s own write (§2.2's acknowledgment-flag analogy doesn't apply here either) is a `track()` call, not a domain write.
- **(Added 2026-08-19, second round) #4 (internal-only entities never leak into user-facing language)** — `demo_sale_completed`/`demo_paid_plan_activated_midsession` are internal/agent-facing identifiers, never rendered in participant-facing copy. No tension.
- **(Added 2026-08-19, second round) #6 (dependency direction)** — both events are pure external observers; neither writes to, branches on, or derives new domain state from what they read.
- **(Added 2026-08-19, second round) #7 (idempotent/keyed retries)** — not applicable; neither performs a domain write, both are read-and-report only.
- **(Added 2026-08-19) Zero PII, count-only or minimal-closed-enum** — consistent with this document's existing "no domain write" posture (§1, §9's #6 entry above), extended here to "reads existing domain facts for reporting only, writes nothing new."
...
- **(Added 2026-08-19) `demo_questionnaire_cta_click` documented retroactively, exactly as built** — a documentation-completeness fix, not a design decision made here; the event itself was already shipped before this amendment existed.
- **(Added 2026-08-19) §1's stale "no analytics/pixel installed" bullet corrected** — the client-side/no-backend distinction is now stated plainly rather than implied by an outdated absolute claim.
- **(Added 2026-08-19) Three new events designed as pure external observers; no Merchant Application component edited** — consistent with D48's placement discipline and this document's own "no domain write" posture, extended here to "no domain write, reads only, for reporting."
- **(Added 2026-08-19) Three-event design held from build authorization pending §8 item 10's delivery-confirmation prerequisite** — Main's own scope call, not an architecture verdict, logged here so a future reader doesn't mistake "designed and reviewed" for "cleared to build."
- **(Added 2026-08-19, second round) `demo_sale_completed` fires per-Sale, not once per session** — the one deliberate departure from the existing four events' one-shot-latch shape, since repeated sale registration during a walkthrough is itself the signal worth counting. Implemented via ID-set diffing, not a raw counter, to correctly exclude Sales already `'finalized'` before this observer's own mount.
- **(Added 2026-08-19, second round; corrected 2026-08-19, `reviewer`) `demo_paid_plan_activated_midsession` deliberately not conflated with `demo_onboarding_completed`'s `path` property** — the two answer different questions (tier chosen at onboarding vs. a later, self-initiated upgrade). Non-conflation isn't just a documentation choice, as first stated: `reviewer` found the originally-specified "saw non-paid → fired" guard, armed on `businessForCurrentUser(state)` being `null`, would have made both events fire for the same underlying fact on the Paid and "Ver un ejemplo" onboarding paths, since `completeOnboarding()` writes `subscriptionTier: 'paid'` directly on those paths with no intermediate free-tier Business ever existing. The corrected guard — armed only when a resolved, non-null Business has `subscriptionTier !== 'paid'` — is what actually keeps the two events distinct in practice, not merely in intent; see §2.5.3's corrected trigger condition.
