...
- **Delivery status, stated plainly (governs §2.5.2's build-authorization gate, §8 item 10): confirmed queuing correctly client-side; NOT confirmed as actually reaching the Vercel Analytics dashboard.** Direct live testing found the event queues (`window.vaq` shows it queued) but was never observed to transmit as a network request. Two undistinguished candidate causes: the Vercel plan tier may lack Custom Events (the Hobby tier excludes them); or the verification session itself may have been filtered as automation/webdriver traffic. Neither ruled in nor out.

#### 2.5.2 Three new events — designed and reviewed, **NOT YET AUTHORIZED TO BUILD** (added 2026-08-19)

**Status: this subsection documents an approved design, not a built feature — matching this document's own precedent for content that has cleared design/review but hasn't cleared its own build gate** (the same shape §8's Architecture Review items use — resolved as guidance before any corresponding code exists). `ux-designer` designed all three; `reviewer` reviewed the design directly against the running code (`ReminderBanner.tsx` confirmed the sole mount point, verified structurally, consistent with `decision-log.md` D48's placement discipline) and found it sound, save for the documentation gap this whole amendment closes. **None of the three is built. None may be built until §8 item 10's prerequisite (`demo_questionnaire_cta_click`'s own delivery confirmation, §2.5.1) resolves** — see the status-header paragraph above.
...
10. **New, blocking §2.5.2's build authorization — a verification prerequisite, not an architecture question.** Whether `@vercel/analytics` custom events actually reach the Vercel Analytics dashboard end-to-end is unconfirmed (`company/merchant-validation-funnel-diagnosis.md` Stage 8) — two undistinguished candidate causes (Vercel plan tier; webdriver/automation filtering on the test session). Until this resolves, `ui-designer` should not build any of §2.5.2's three events — they'd share the identical unconfirmed delivery mechanism, and building three more events on an unverified delivery path would compound, not reduce, the uncertainty `reviewer` flagged. This is Main's own holding call (stated in the status header above), not an Architecture Review verdict — no new architectural question exists here for `architect` to resolve; it's a data/ops verification task (confirm Vercel plan tier; retest with a real, non-automated tap) outside this document's own scope.
