...
- **BusinessMembership** — root, Identity context, **not** an entity nested inside Business — fails the internal-only-entity test (below) the same way `Venue` did: resolving "which Businesses can this User act in, and with what role" requires querying by `userId` alone, independent of any single Business, before any one Business is even in context. Schema: `id`, `userId`, `businessId`, `role` (`OWNER` | `SELLER` — closed set, not merchant-extensible, same treatment as `Event.type`, D16), `createdAt`. Unique on `(userId, businessId)` — a User holds at most one Membership per Business, same class of dedup invariant as `Customer`'s `(businessId, email)` (D37). The only write path that can create a Business atomically produces its `OWNER` Membership in the same transaction — see "Owner-creation invariant" under Key Mechanisms. `SELLER` Memberships are unreachable by construction until the future invitation flow exists. See `decision-log.md` D44.
...
User (phone, phoneVerifiedAt)
  └─ BusinessMembership (role: OWNER | SELLER) ──→ references Business
       (the OWNER Membership is created atomically with its Business —
        no Business exists without one; see Key Mechanisms, D44)
...
|---|---|---|---|
| **Identity** | Business, Capabilities, User, BusinessMembership | *configure* | — |
