...
- **Settings**: reachable from the existing session-controls affordance already specified in `home.md` §3.3–§3.7a (the header's "▾"), reachable from every Home state with a persistent header — not a dedicated tab. This keeps capability management consistent with "selling is a state, not a navigation destination" — settings hang off an existing surface rather than adding a fifth persistent destination.
