...
- Standard pipeline (React-first as of D42): **Architect → UX Designer → UI Designer → UX Critic → Reviewer → Builder.** Sequencing/prioritization is Main's own standing work (the `backlog-prioritization` skill), not a separate pipeline stage. UX Designer produces Low-Fidelity specs (`product/02-ux/`, also covering the Product Definition/UX Flow Review stages of the standing feature workflow — see `company/CLAUDE.md`); UI Designer builds the feature directly as a React/TypeScript living prototype (`product/02c-high-fidelity-prototype/`, the standing default execution medium) on top of an Approved Low-Fidelity spec and an Approved architecture review — Figma is no longer a required intermediate stage, retained only for the narrow optional cases named in `company/CLAUDE.md` (brainstorming, concept exploration, marketing assets, diagrams). A screen may pass through UX Critic and Reviewer more than once as the same running prototype evolves, not once per fidelity tier. Each agent's own file (`.claude/agents/`) defines its exact remit and how it hands off to the next.
- The **Architect** protects the foundation — new ideas are checked against it before being sequenced or before `Builder` implements them.
