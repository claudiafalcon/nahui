...
**What's now genuinely confirmed working end to end, live, against the real hosted backend:** the pre-auth `peek-invitation` path for a dead/nonexistent token, exactly matching spec. **What's still not verified live** (no real Business exists anywhere in this project's real database — confirmed via direct query, zero rows — so nothing has ever completed real Onboarding against this backend): the "happy path" of a genuinely `pending` Invitation resolving to the real offer screen with a real business name; the full accept flow through any of the three authentication methods; `send-otp`/`verify-otp` end to end (still additionally blocked on missing Twilio credentials for the former). `tsc -b`/`npm run build` clean throughout. Not yet `reviewer`-verified — a review pass covering all of this (the two grant migrations, the corrected header comments, the client-side fix) is the next step before this is committed.
