...
## Slice 18 — `User.displayName` (Q29) and `Invitation.targetHint` enforced (Q30), written but NOT YET PUSHED

Two new migrations, both **written and reviewed against every existing precedent in this file, but not applied to the real hosted project** — this environment has no `SUPABASE_ACCESS_TOKEN` (confirmed by running `supabase projects list`, which returned `LegacyPlatformAuthRequiredError`), the same CLI-auth gap `company/infrastructure-decisions.md` ID018 already names for every prior pass that hit it. **This also means this pass could not comply with the "execute before done" discipline this file's own most recent entry establishes for SQL** — every function below was written against the exact patterns already proven live elsewhere in this file (`update_business_identity`/`cancel_invitation`'s own shape for the new RPCs; `accept_invitation`'s own existing CAS structure for the mismatch-check restructuring), and `npm run build` (`tsc -b && vite build`) is clean against the client-side code that calls them, but neither migration has been executed — via `BEGIN`/`ROLLBACK` or otherwise — against the real hosted project. **Main (or whoever next holds a working legacy access token) must run both migrations' own functions through a real `BEGIN ... SET LOCAL request.jwt.claims = ...; SET LOCAL role authenticated; ...; ROLLBACK` pass before trusting them live**, the same standard this file's own prior entry sets, not skipped here by choice.
...
**Push checklist, once a working access token exists** (same CLI-auth workaround as every prior push in this file):
1. `supabase db push` (applies both new migrations).
2. Real-execution verification pass against both new/modified RPCs (`update_user_display_name`, `update_invitation_target_hint`, `create_invitation`, `accept_invitation`, `peek_invitation`) — this file's own established discipline, not optional.
3. `supabase functions deploy peek-invitation`.
4. Live-verify: an OWNER can set her own name in Configuración and see it reflected in "Tu equipo"/"Vendiendo ahorita"/an export from a *second* authenticated session (the actual cross-device read this pass's `hydrateFromBackend` change exists to serve — nothing in this environment can verify that round-trip without two real, separately-authenticated sessions); "Nueva invitación" rejects a blank email server-side even if a client bug ever let one through; an Invitation-acceptance attempt under the wrong email genuinely raises `invitation_identity_mismatch`, not `invitation_not_available`.
