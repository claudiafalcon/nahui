-- Stage 7 Backend Integration — real `cancel_invitation` RPC, closing the
-- `settings.md` §3.12d "Cancelar invitación" gap: the UI (`TeamScreen.tsx`'s
-- cancel-confirm sheet) was already fully built against a local-mock
-- `cancelInvitation` store function; this migration adds the real backend.
--
-- Design notes (architect):
--
-- CAS precondition is `status = 'pending'` only, deliberately
-- expiry-independent — `settings.md` §3.11 hides the "Cancelar" button on an
-- expired row for UX reasons only, not a domain rule, matching
-- `regenerate_invitation`'s own final "any still-pending row" precondition
-- (`20260914032000_invitation_token_no_raw_cache.sql`).
--
-- No Paid-tier gate, deliberately — unlike `create_invitation`/
-- `regenerate_invitation` (which mint a new capability a downgraded
-- Business shouldn't get), `cancel_invitation` only ever closes an existing
-- exposure; gating it would strand a downgraded OWNER unable to kill a
-- leaked link with no offsetting benefit.
