...
**`commitLot()`/`editPrice()`/`setProductPhoto()` closed 2026-09-13 — Stage 7 Phase 1 (Inventory).** All three now call real, idempotency-keyed (`crypto.randomUUID()`) Supabase RPCs (`commit_lot`/`update_product_price`/`update_product_photo`), matching the exact `insert ... on conflict (id) do nothing` pattern Phase 0 established. `assignTagToNextPendingUnit()` (not named above, but the same gap class) closed in the same pass via `assign_tag_to_next_pending_unit`. **`commitAllocation()`/`releaseAllocation()`/`saveEventAllocations()` remain open** — Selling-context writes, owned by Phase 2/2b, not touched by Phase 1.

**Adjacent gap found and closed 2026-09-13 (`ui-designer`, D65 barcode-scanning build) — `commit_lot`'s own `products.barcode` write was missing entirely, not just unguarded.** The Phase 1 migration above added the `barcode` column and its partial unique index but never named the column in `commit_lot`'s `insert into public.products`, so a scanned barcode would have been silently discarded on write. Closed via a new, additive migration (`20260913032000_inventory_barcode_write.sql`) that writes `barcode` when present and re-raises a genuine collision as `barcode_already_registered`. **Not yet pushed to the real hosted project** — no CLI-auth credential in this build environment; see `supabase/README.md` checklist item 12 and `docs/passes/slice-14-barcode-scanning.md` for the full record.

**`commitLot()`'s own closure corrected 2026-09-13 (`reviewer` Blocker, same-day fix round).** The line above ("idempotency-keyed (`crypto.randomUUID()`)") understated the actual gap this entry exists to track: `commit_lot`'s client wiring minted a *fresh* `crypto.randomUUID()` on every call, including a retry, which defeats the RPC's own replay-on-conflict guarantee the same way an unguarded write would — a lost-response retry could still mint a duplicate Lot/Products/InventoryUnits. Fixed: each caller (`RegisterMerchandise.tsx`, `OnboardingFlow.tsx`'s `SellingGroups` wrapper) now owns a stable `useRef<string | null>` key, generated once per logical attempt and reused across any retry of that same attempt — the identical shape `completeOnboarding()`'s own `onboardingIdempotencyKeyRef` already established, mirrored per caller rather than shared, since `commit_lot` has multiple independent call sites. `editPrice()`/`setProductPhoto()` were never affected by this specific defect — a plain single-column `UPDATE` has no duplicate-creation side effect a repeated key would need to guard against, so a fresh key per call is genuinely harmless there (see `update_product_price`/`update_product_photo`'s own migration header).
