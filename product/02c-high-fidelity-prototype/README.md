...
Selling's tile grid, alongside typed search/tile taps; a live,
Business-wide, cross-Session view of every currently-active Session
("Vendiendo ahorita," Paid tier); and "Exportar tus ventas," a
date-range-picked, client-side CSV export of Ana's own sales, available
at any tier.
...
- **[`docs/passes/slice-15-quick-session-alongside-event.md`](docs/passes/slice-15-quick-session-alongside-event.md)** — `home.md` §3.6/§3.6b (Product Owner-raised, live production testing, 2026-09-15): a second, secondary, always-available "Iniciar Venta Rápida" action on `EventResume.tsx`/`ElegirEvento.tsx`, opening a Quick Session (`eventId = null`) independent of whichever Event(s) are active — closes a real gap (no Quick Session path existed once a qualifying Event went `active`) grounded in `architecture-principles.md` #3 and the Product Owner's own tianguis-length-Event observation. Reuses `Idle.tsx`'s existing primary action verbatim (same `startSession` call, same error/retry states, same ambient NFC-readiness hook) — no new interaction pattern. A qualifying caption directly beneath the secondary CTA states its independence as a plain functional fact. `ux-critic` found and closed 2 Major (the caption's visual binding to its CTA; a missing scroll affordance for the fuller composition) across one remediation round. `reviewer` found 0 Blockers (1 Important — this slice's own missing pass/README entry, closed by this same edit). `tsc -b`/`npm run build` both clean. No live browser/device verification was possible in this environment — the `safe center` scroll-fallback fix in particular is verified structurally only.
- **[`docs/passes/slice-16-vendiendo-ahorita.md`](docs/passes/slice-16-vendiendo-ahorita.md)** — `reports.md` §2/§3.3/§3.4a/§3.4b (Product Owner-raised, live production testing, `decision-log.md` D68, 2026-09-15): a live, Business-wide, cross-Session, cross-device view of every currently-`active` Session ("Vendiendo ahorita"), rendering above every other Resultados state — an OWNER checking in on several SELLERs at once, without any Session closing first. New `activeSessionsForBusiness`/`membershipById` selectors; new `VendiendoAhorita.tsx`/`VendiendoAhoritaDetail.tsx` (genuinely read-only — no affordance anywhere resumes, closes, or acts on a Session); `ResultadosColdStart.tsx` gains a live-aware copy variant so it never asserts "you haven't started selling" beneath a card proving otherwise. `ux-critic` found and closed 1 Blocker + 1 Major across two remediation rounds — "Total histórico"/"Top productos"/the sales-trend line, and separately `eventRollup` (Historial/Event/Venue detail), both silently counted a live Session's Sales as settled, contradicting §2's own new rule; both now correctly exclude them via a shared `activeSessionIds` helper. One narrow, self-correcting Minor left open and disclosed (`eventCompletedDays` can count a still-open day after its Event's date range has elapsed). `reviewer` found 0 Blockers, 0 Important (independently confirmed no write path exists anywhere in this new code, and role resolution degrades safely with no crash). `tsc -b`/`npm run build` both clean. No live browser/device verification was possible in this environment. **Further amended 2026-09-15, same night (Product Owner decision, `product-decisions.md` Q28): Paid tier only, not free-tier-and-paid-alike as shipped above.** `reports.md` corrected to match (§2's live-session check now ANDs `subscriptionTier=paid` with the existing active-Session check); the React code gated the same way — `ResultadosMain.tsx`'s `<VendiendoAhorita>` mount and `ResultadosColdStart.tsx`'s Variant B selection both now require `subscriptionTier=paid`, reusing the file's own pre-existing `paid` const (the same one already gating "Rendimiento por bazar"/"Tus clientes"). `ux-critic` found and closed 1 Blocker on the spec correction itself (the canonical wiring definition, §4, plus five other cross-references still listed the Free-tier-only screens as valid render targets) and, on the code gate, 0 Blockers/Majors/1 Minor (a narrow, disclosed, self-correcting gap — see the pass file's own Known Limitations). `reviewer` found 0 Blockers throughout — 2 Important documentation-persistence gaps (this line and the pass file's own missing correction record; a missing `ux-critic-findings.md` entry), both closed by this same edit. `npm run build` re-confirmed clean.
- **[`docs/passes/slice-17-exportar-ventas.md`](docs/passes/slice-17-exportar-ventas.md)** — `reports.md` §3.19/§3.20 (Approved 2026-09-15, `product-decisions.md` Q27, Product Owner-requested), plus the new "[ Exportar tus ventas ▸ ]" row wired into §3.4/§3.5/§3.6's existing main-view screens: a single combined CSV export (one row per `(Sale, Product)` pair within a merchant-picked date range, "ID de venta" repeated across a multi-item Sale's own lines), purely client-side — `architect`-confirmed additive, no new backend read. Available at any `subscriptionTier` (unlike "Vendiendo ahorita"/"Rendimiento por bazar"/"Tus clientes"), OWNER-only via the tab's existing role gate. New selectors (`closedSessionsInRange`, `defaultExportRange`, `salesExportRows`) reusing `dayNumberForDate`/`membershipById` verbatim from the "Vendiendo ahorita" slice; new `salesExportCsv.ts` (CSV formatting + Blob/`<a download>` trigger, this build's own already-disclosed "Venta rápida" rename applied consistently in the Sesión column); new `ExportarVentas.tsx` (one component covering both the §3.19 range picker and the §3.20 generate/download step, the same internal-sub-state-machine shape `TeamScreen.tsx` already uses). `ResultadosMain.tsx` gains the new row as a plain secondary `Button` (matching §3.10's existing "[ Ver Eventos ]" precedent, not the two-part `.teaserRow` shape the paid-only sections use); `ResultadosScreen.tsx` gains a new `{ mode: 'export' }` view, no tier re-check needed. `tsc -b`/`npm run build` both clean. No live browser/device verification was possible in this environment. Review Pipeline (`ux-critic`/`reviewer`/`merchant-user-tester`) has not run against this build yet.
- **[`docs/passes/slice-18-displayname-target-hint.md`](docs/passes/slice-18-displayname-target-hint.md)** — `decision-log.md` D69 (`product-decisions.md` Q29, `User.displayName`) and D70 (RFC 0014, `product-decisions.md` Q30, `Invitation.targetHint` required and enforced at acceptance), both Approved 2026-09-15. Q29: new self-service `User.displayName` field, captured either at Onboarding's "Tu negocio" (a second, independently-sequenced, non-blocking write, `BusinessIdentity.tsx`/`OnboardingFlow.tsx`) or via a new "Tu nombre" row/edit sheet in **both** "Tu cuenta" surfaces this build has — `SettingsScreen.tsx` (OWNER) and `SellerAccountScreen.tsx` (SELLER, `HomeScreen.tsx`'s own pre-existing role-based stand-in a SELLER actually reaches instead of Configuración — the load-bearing half of this pass's own scope check, since D69's SELLER capture surface would otherwise be unreachable in the built app). Display resolution (`User.displayName` → role-only fallback) now runs through a shared `membershipDisplayName` selector, reused by "Vendiendo ahorita," the sales export's Vendedor column, and "Tu equipo"'s own three-tier (`displayName` → phone → role) row identity. Q30: `settings.md` §3.12's invite-creation email field is now required (`TeamScreen.tsx` gates "Generar invitación" on it, distinguishes an ordinary hint-bearing pending row from a legacy hint-less one, and adds a new §3.12e "Editar correo"/"Agregar correo" repair sheet); `authentication.md` §3.2g (new `LockedEmailStep.tsx`, wired into `AuthenticationFlow.tsx` via new `lockedInvitationEmail`/`onDeclineInvitation` props that skip the ordinary three-way method choice entirely) and §3.10f (`InvitationFlow.tsx`'s new `'mismatch-account'` step) are both built. Two new Supabase migrations (`public.users`/`update_user_display_name`; `create_invitation`'s required-hint check, `accept_invitation`'s new `invitation_identity_mismatch` precondition + an idempotency-replay bug fix found along the way, `update_invitation_target_hint`, `peek_invitation`'s widened return shape) plus a `peek-invitation` Edge Function change are **written but not pushed/deployed** — no working `SUPABASE_ACCESS_TOKEN` in this environment; full disclosure and push checklist in `supabase/README.md`'s own new "Slice 18" section. `tsc -b`/`npm run build` both clean. No live browser/device verification was possible in this environment. Review Pipeline (`ux-critic`/`reviewer`/`merchant-user-tester`) has not run against this build yet.
...
- **NFC becomes a live, composable selling capability (2026-09-18, `ui-designer`, `decision-log.md` D79 / `product/99-rfc/0017-nfc-composable-selling-capability.md`) — retires `Session.operatingMode` and NFC Readiness outright.** Closes the Product Owner's own live-testing finding: a Session opened before any unit was tagged never offered "Leer con NFC" for that unit, even after tagging it mid-Session, because `Session.operatingMode` resolved once at Session-open and NFC Readiness's threshold gated the overlay to Limited Ready specifically. Both mechanisms are removed, not re-thresholded. `Session.operatingMode`/`SessionOperatingMode` dropped from `types.ts` entirely (no replacement field); `Business.defaultSellingMode` dropped from the client `Business` type entirely — `hydrationMapping.ts` no longer maps either DB column onto the client shape, `store.tsx`'s `startSession`/`completeOnboarding` no longer resolve or write either. `selectors.ts` loses `nfcReadiness`/`NFC_READINESS_THRESHOLD`/`NfcReadiness`/`taggedAvailableCount`/`totalAvailableCount` outright (zero remaining callers, confirmed by trace). `Selling.tsx`'s selling surface collapses to one unconditional composition for every Session — the buttons grid always, the barcode overlay (`subscriptionTier=paid`) and "Leer con NFC" overlay (`nfcPerProductEnabled===true` alone, no Session/readiness gate) both re-evaluated live on every render — the former full-screen, whole-catalog `nfc`-only surface (old `home.md` §3.10) is deleted along with its own resolver (`resolveTag`), since the composable overlay is a strict superset. `home.md` §3.6a's Session-start NFC-readiness sub-step (Limited Ready override, Not Ready, capability-revoked) is retired in full — `useNfcSessionStart.ts`/`NfcSessionStartNote.tsx` deleted, `Idle.tsx`/`EventResume.tsx`/`ElegirEvento.tsx` all corrected (no more `overrideToNfc` threaded through `onStartSession`/`onContinue`/`onStartQuickSession`, no more dead `onOpenAssignTagsPlaceholder` prop). `settings.md` §2.3 "Cómo vendes normalmente" (status line + "Cambiar a vender con botones"/"...con tags") is removed entirely from `SettingsScreen.tsx` — "Activar NFC"/"Desactivar NFC" is now the sole NFC-related control, unconditionally, for every Paid-tier Business. **Server-side companion fix, same migration** (`20260918030000_d79_nfc_composable_selling.sql`, **written but not yet pushed** — no working `SUPABASE_ACCESS_TOKEN`/CLI login in this environment): `add_item_to_sale`'s `wrong_operating_mode` rejection (fired whenever `operating_mode <> 'buttons'`) and `add_item_to_sale_by_tag`'s `no_match`-via-`operating_mode <> 'nfc'` rejection are both removed — these were the actual server-side enforcement of the exclusive-mode split the client-side gate fix alone can't bypass. `start_session`'s 3-arg signature (`p_operating_mode` required, no default) is dropped and replaced with a 2-arg version; `sessions.operating_mode` gains a database-level default (`'buttons'`) so no INSERT has to supply a value. **Deployment-ordering risk, flagged explicitly:** this client code now calls the 2-arg `start_session` RPC — until the migration above is pushed, "Iniciar Venta Rápida" will fail outright against the live database (the currently-deployed 3-arg function has no default for `p_operating_mode`). The migration must be applied before/atomically with this client deploy, not after. A found-and-disclosed, not silently patched, entanglement: `onboardingResolution.ts`'s `pathFromCapabilities` used to reconstruct the `'demo'` Onboarding path from `(subscriptionTier, defaultSellingMode)` alone (an implementer shortcut, never part of any Approved spec) — with `defaultSellingMode` gone, it now matches on `Business.name === DEMO_BUSINESS_NAME` instead (the demo path's own atomic identity write, which always lands before `path` is ever actually consulted downstream) — a low-risk substitute, not a re-reading of the retired field, but named here as a real follow-up candidate for `ux-designer`/`architect` if a more honest signal is ever wanted. `tsc -b`/`npm run build` both clean. No live browser/device verification was possible in this environment (the deployment-ordering risk above means a live retest can't succeed until the migration is pushed). Review Pipeline (`ux-critic`/`reviewer`) has not run against this build yet.
  ...
  src/
    ...
    domain/
      ...
      selectors.ts             — pure derived reads (catalog rows, selling
                                 ...
                                 Slice 12; activeSessionsForBusiness/
                                 membershipById — Slice 16;
                                 closedSessionsInRange/defaultExportRange/
                                 salesExportRows — Slice 17)
    ...
    screens/
      ...
      Resultados/                 — ResultadosScreen ({mode,...} resolution,
                                 ...
                                 VendiendoAhorita/VendiendoAhoritaDetail
                                 (§3.4a/§3.4b, Slice 16), ExportarVentas
                                 (§3.19/§3.20, Slice 17 — the range picker
                                 and generate/download steps in one
                                 component), salesExportCsv.ts (Slice 17,
                                 CSV formatting + Blob/`<a download>`
                                 trigger, presentation-layer, not
                                 domain/selectors.ts)
  docs/
