...
`product/02-ux/settings.md`, Approved; amended for `decision-log.md` D27 (NFC capability corrected to derive from `subscriptionTier`, not kit/code activation — the "Activar venta con tags" activation path is retired, a new `defaultSellingMode` control is added). Medium-Fidelity work reflects that current, D27-amended state as of this rebuild.

## 2. Figma file/frame links
File `DPRnGD5JWjfoNBSlAFoVG4` ("Nahui — Medium-Fidelity UI"), page **"Configuración — Medium Fidelity"** (`74:2`). All 10 `settings.md` §5 states covered across 17 frames (§3.4's five copy variants and §3.9's near-instant/slow sub-states each split into their own frame, matching this phase's established convention):
...
| `74:8` | §3.4 Activar clientes frecuentes |
| `74:9` | §3.4 Desactivar clientes frecuentes |
...
- **`CapabilityCard`/`CardStack` consolidated into one real Design System component (`236:3404`, page `0:1`), 2026-08-04.** 8 variants (capability × state) replace 12 previously-duplicated ad-hoc frames across this file and `home.md`'s tracking. One variant — `Clientes/Encendido` ("Clientes frecuentes: Encendido" / "Desactivar clientes frecuentes") — exists only on the Design System page; no live screen instance in this file currently demonstrates the paid + clientes-frecuentes-encendido combination together (mirrors a gap already present in the approved low-fi spec itself, not a regression). `ux-critic` verified the completed copy is a faithful, non-inventive extrapolation of `settings.md` §2.2/§3.4's existing action-name vocabulary. Noted here for traceability since no live frame currently references it.
