...
### 2026-08-10 — Demo sample-data replacement: "Ropa Ana"/Pijama/Sudadera-Maxy/Calcetines → "Luna Mercado"/Bolsas/Accesorios/Playeras

Product Owner decision: the public demo shouldn't keep using the real pilot merchant's actual business/product mix now that it will be shown to multiple potential users. Business identity and the three illustrative product names/prices were replaced with clearly fictional, neutral data across every page — pure text-content substitution, no layout/reaction/component-structure change, explicitly not a Product/Foundation concept change. Already applied at the Low-Fidelity spec layer first (`product/02-ux/home.md`/`onboarding.md`/`inventory.md`/`events.md`/`reports.md`, `product/02-ux-loyalty/customer-loyalty-registration.md`).

**Substitution map:** "Ropa Ana" → "Luna Mercado"; "Pijama" → "Bolsas" ($150→$350); "Sudadera/Maxy" → "Accesorios" ($220→$180); "Calcetines" → "Playeras" ($60→$280). A stray illustrative 4th tile, "Bufandas," was renamed to "Gorras" to avoid a per-product marker-letter collision once "Bolsas" took the "B" — same fix applied at both fidelity layers.

**Build.** 229 text nodes edited across 6 pages (Home 27, Inventario 47, Eventos 8, Resultados 26, Onboarding 9, Demo — Journey 1 Seamless 112; Configuración needed zero changes, confirmed clean both before and after). Two real gaps self-caught mid-build, both only visible by reading actual `.characters` rather than trusting `get_metadata`'s node *names*: "Ropa Ana" initially reported as zero-hit turned out to be real rendered text in 4 nodes with generic layer names ("Field Nombre value," etc.); the "Bufandas" collision was confirmed to live inside `ProductTile` instances' single `ProductName` text child, not a separate badge element as first assumed. Hit the standing ID001 Figma-skill-loading gap before any write — resolved via the *original* ID001 mechanism (Main reading the skill resource directly and inlining it), after a first attempt using a since-superseded local-cache fallback was correctly refused by `ui-designer` itself, which caught that Main had overstated the precedent.

**Verification.** Final sweep: fresh, live `.characters` readback (not layer-name inference) on all 7 pages returns zero remaining old-value hits anywhere. No `ux-critic`/`reviewer` cycle run — pure content substitution with no design judgment involved, same standard this project already applied to prior static-content swaps (2026-08-06 ruling).

### 2026-08-10 — Paid-tier "Iniciar Sesión Rápida" selling/receipt reconvergence: found and fixed, same defect class as the 2026-08-07 Onboarding-native reconvergence

A `merchant-user-tester` walk of the full Paid acceptance journey completed a sale that landed on a receipt explicitly self-titled "...modo botones, plan gratis... [Free-tier fix]" despite having activated the Paid plan at the very start — plus Configuración showing "Tu plan: Gratis" again. Both symptoms traced to one root cause via a `ui-designer` ground-truth `node.reactions` trace: `658:2766` (the Paid-tier "Iniciar Sesión Rápida" nav-diverge Home clone) had been correctly cloned for its own Configuración leg back in the original reconvergence fix (`658:2777`/`658:2785`, above), but its `ProductTile` was left pointing at the shared, original, un-cloned buttons-only chain — `162:1511` ("Venta en curso") → `162:1519`/`162:1521` ("Finalizar Venta"/"guardando") → `825:2941`, the Free-tier receipt (confirmed via content: no `ClaimTokenQR`/`ClaimTokenCaption`, unlike the Paid/NFC receipt `284:3552`). Every one of `825:2941`'s own exits (auto-dismiss, `MarginTapZone`, "Hoy" tab) returned to `162:1500` (the Free hub), not back to `658:2766` — so a Paid session reaching this leg silently and permanently dropped into the Free-tier subtree with no QR ever shown. Same two-axis-classification gap (D31/D32) already fixed for this exact leg's other legs, just never applied to the selling leg itself.

**Fix.** New clones, all on this page: `936:3110` (clone of `162:1511`), `936:3120` (clone of `162:1521`), `936:3125` (clone of `825:2941`, composed with `937:5910` ClaimTokenQR + `937:5993` ClaimTokenCaption — both reused verbatim from `284:3552`, not invented; `BrandMark` repositioned y=460→410 to match the Paid-tier layout convention already established by `284:3552`/production `192:382`). `658:2766`'s `ProductTile` (`658:2770`) rewired to `936:3110`; the new clone's own exits (auto-dismiss, `MarginTapZone`, "Hoy" tab) all point back to `658:2766`, not `162:1500`. `162:1511`/`162:1519`/`162:1521`/`825:2941` and their other 9 legitimate callers left completely untouched — confirmed via a before/after caller-audit sweep.

**Verification.** `ux-critic`: 0 Blockers/Major/Minor/Suggestions — direct pixel comparison confirmed `936:3110`/`936:3120` content-identical to source and `936:3125`'s QR composition pixel-consistent with `284:3552`'s own paid-receipt layout; correctly named the reaction/wiring-level claims as an open boundary outside its tools (ID004), given this exact artifact's documented history of receipt-exit-wiring bugs (`home.md`'s own prior "Real Blocker found and fixed" entry for `192:382`/`231:1920`/`198:823`/`284:3552`'s original exit wiring). Main closed that boundary directly via live `chrome-devtools-mcp` click-through on a fresh page: `658:2766` → tap product tile → `936:3110` → "Finalizar Venta" → `936:3120` → `AFTER_TIMEOUT` → `936:3125` (QR visually confirmed rendering via screenshot) → `MarginTapZone` → back to `658:2766` with a genuinely empty tray ("Venta actual: (vacía)"), not a leftover state. `reviewer`: 0 Blockers — confirmed the fix correctly extends D40's "Multi-mechanism Claim resolution" rule (Sale-level Claim Token generated at every Sale finalization for a Paid-tier Business "regardless of `registrationMode`," which explicitly covers this buttons-mode receipt), no ubiquitous-language drift, remediation technique matches the D31/D32-sanctioned clone-don't-share pattern exactly. One Important finding — this fix had no tracking-file/bitácora record before reaching review, the identical documentation gap `reviewer` caught in the original reconvergence workstream above — closed by this entry.

**Status:** fully closed — built, `ux-critic`-clean, wiring-verified live by Main, `reviewer`-clean. A fresh `merchant-user-tester` walk of the full Paid journey from the beginning is the standing next step per this project's "rerun from the beginning, never resume" rule, not yet dispatched as of this entry.

### 2026-08-12 — `184:1503` family reconvergence: a fourth Catalog-routing instance plus two more legs of the same defect class, all on the primary "Activar plan de pago" onboarding path

A fresh, isolated `merchant-user-tester` walk of the Paid journey (clean, no contamination — see `company/infrastructure-decisions.md` ID015) went through `184:1503` itself — the primary onboarding Home landing, distinct from the `658:2766` "Iniciar Sesión Rápida" family fixed above — and found three more real defects, all confirmed via ground-truth `node.reactions` trace before any fix:

1. **Inventario Catalog-routing, a fourth unaudited instance of the same `inventory.md` §2 step-1 violation closed for `162:1492`/`284:3698`/`36:61`/`111:1085` on 2026-08-11/12** (see the Inventario Catalog-routing entry in `product/02b-medium-fidelity/inventory.md`). `184:1503`'s Inventario tab pointed unconditionally at `651:2646`, the blank entry form — even after real merchandise was registered through this exact chain — because this leg was never in scope of that earlier pass (it audits by "Día 2 active" content, and `184:1503` is the Idle/no-event state, a different frame family). **Fix:** cross-page cloned production `46:29` → new node `981:620`; `184:1503`'s Inventario tab (and a sibling's, `524:4628`'s own Configuración-sheet Inventario tab, found and fixed proactively during the caller-audit) rewired to it; the clone's own "Registrar mercancía" button rewired to `651:2646`, unchanged.
2. **Missing QR on the receipt reached via this leg's own Eventos-created-event selling chain, same class as the `658:2766` fix above, a different, never-audited leg.** `184:1503`'s Eventos tab led, after creating an event, to `162:1900` — a confirmation screen legitimately shared with the real Free/buttons-only Journey 1 — whose "Hoy" tab reconverged onto `162:1492`, Journey 1's own hub, terminating on the identical Free-tier `825:2941` receipt with no QR. Since `162:1900` has genuine Free-tier callers that can't be touched, the fix cloned one level earlier: new chain `988:3196` (cold start, clone of `162:1804`) → `988:3202` (form, clone of `162:1877` — same populated-Tipo/blank-Lugar state the 2026-08-07 NFC-cold-start precedent already established for this exact class of problem) → `988:3225` (saving, clone of `162:1896`) → `988:3229` (confirmation, clone of `162:1900`), with every exit closing back into `184:1503`'s own world. Reusing `658:2766` as the target was investigated and rejected — it shows an already-open selling session, which would silently skip the "start selling" moment for a merchant who just scheduled her first event.
3. **The family's own tag-queue chain bled into an unrelated, older persona.** "Guardar mercancía" routed through shared nodes (`475:1925`→`284:526`→`284:535`) also used by the 2026-08-05 "Connected NFC activation-to-sale chain" persona — reused, not recloned, at build time in the original 2026-08-07 fix (disclosed then, never revisited). Two hops later, this leaked into that persona's own unrelated Home/session content ("Plaza Norte · Día 2 · $850 · 6 ventas," never created by whoever was actually on `184:1503`'s path) and a stray QR-bearing receipt belonging to that other persona (`284:3552`, not this leg's own). **Fix:** new dedicated chain `984:3126` (saving, clone of `475:1925`) → `984:3129` (tag queue, clone of `284:526`) → `984:3138` (confirmation, clone of `284:535`), with every exit — including a proactively-found "Terminar después" leak — closing back into `184:1503`'s own world instead. `475:1925`'s other 10 legitimate callers (unrelated nfc-Home origins) confirmed untouched.

**Small correction, same pass:** `651:2684` ("← Inventario" on the "Elegir producto" picker sheet) pointed at `184:1503` instead of `668:526`, unlike its three siblings — repointed to match.

**Verification.** `ux-critic` (elevated scrutiny — the build agent disclosed it couldn't render its own screenshots this session, no Bash/curl access, so a genuine visual pass was explicitly required): found two Majors. One — `988:3202` visually shows "Guardar evento" disabled-styled with Lugar unset, contradicting the build's own description — **resolved by Main via live click-through**: the reaction fires regardless of the visual disabled state (`988:3202`→`988:3225`→`988:3229`, reaching "Evento agendado ✓ · Plaza Norte"), and "Plaza Norte" appearing despite Lugar never being visibly set is the same static-content convention `162:1900` (the clone source) already carries, not a new defect. The other — reusing the Tipo-preselected/Lugar-blank deviation (previously reviewed only for a different, experienced-mid-session context) at the *true* first-time Eventos cold start reads as a field answering itself with no explanation — **accepted as a provisional, disclosed limitation**, matching this project's own established pattern of shipping a real fix while naming and deferring a narrower adjacent UX question (same class as exception #6, Finding 10's deferred nodes). Follow-up flagged, not resolved today: build the literally-blank-form leg instead for this specific first-time context.

`reviewer`: zero `decision-log.md`/ubiquitous-language violations, D31/D32 clone-don't-share discipline confirmed correctly applied (every new node's own layer name states its clone lineage explicitly), both `ux-critic` Majors' resolutions confirmed sound. **One further, real Important finding:** `981:620` inherited real quantities verbatim from its clone source `46:29` — but `184:1503`'s own narrative (per `onboarding.md` §4 and `home.md` §2 step 3) is a merchant who has named Products but never registered a single Lot, the exact scenario `inventory.md` §3.4 already has a convention for ("sin registrar," not a quantity). Routed to `ui-designer` as its own small content-only fix, not decided by `reviewer` or Main directly. `reviewer` also separately flagged, as a deeper pre-existing question predating today's fixes and explicitly not resolved here: whether `184:1503` itself sits on the correct baseline state at all (labeled "Idle"/§3.4, where `onboarding.md` §4's routing appears to call for "Cold start"/§3.3) — named for a future `ui-designer` verification pass, not chased today.

Main live-click-verified every primary path end to end on fresh pages, including the exact hop the original `merchant-user-tester` finding reported broken: `184:1503`→Inventario→`981:620` ✓; `184:1503`→Eventos→`988:3196` ✓; the full registration path (`651:2646`→picker→"Guardar mercancía"→`984:3126`→`984:3129`→`984:3138`, correct quantities) ✓; and `984:3138`'s own Inventario tab→`981:620` ✓ — the precise chain the tester walked and found broken.

**Status:** fully closed — `981:620`'s three rows corrected to "sin registrar" (matching `inventory.md` §3.4's own established convention exactly, no new visual state), visually confirmed by Main via live screenshot. All three defects built, `ux-critic`-clean, `reviewer`-clean, wiring-verified live end to end by Main. The standing next step is a fresh, isolated `merchant-user-tester` rerun of the full Paid journey from the beginning.

### 2026-08-12 — `184:1503` family reconvergence, round 2: a dual-caller content conflict (self-inflicted regression) plus a fifth, never-audited reconvergence leg

A fresh, isolated `merchant-user-tester` rerun of the full Paid journey (per the standing next step above) came back clean on the primary path, but surfaced three further items on a deeper walk:

1. **`981:620` dual-caller content conflict — a regression introduced by round 1's own "sin registrar" fix.** The round-1 fix (above) correctly corrected `981:620`'s content for its then-known callers, but `981:620` actually had 5-6 real callers, some pre-registration (correctly "sin registrar") and some reached only *after* real merchandise registration (`984:3138`'s Inventario tab and "Ir a Inventario" CTA; `988:3229`'s Inventario tab, reached via the family's own Eventos-created-event chain) — those post-registration callers now incorrectly showed "sin registrar" instead of the real quantities they'd shown before round 1's fix. **Fix:** cloned `981:620` → new node `1014:3292` (real quantities: Bolsas 10/Accesorios 5/Playeras 20 disponibles, reusing `593:526`'s own quantities). Repointed the confirmed post-registration callers (`I984:3139;2:36`, `984:3152`) to `1014:3292`; left the confirmed pre-registration callers (`I184:1509;2:27`, `I988:3201;2:45`, `I524:4635;2:27`) on `981:620`.

2. **A fifth, never-audited reconvergence leg — `185:699`, reached via the family's own Eventos EventCard (`988:3234`).** `185:699` is a pre-existing node shared with the original, unrelated Eventos-list EventCard (`162:1905`) — its own downstream chain is the Free-tier baseline (`162:1492`→...→`825:2941`, no QR, "Tu plan: Gratis"), wrong for this Paid-tier family. **Fix, investigated reuse-vs-clone per the D31/D32 two-axis test before building anything new:** content and navigation needs were both identical to the already-fixed `658:2766` chain (built 2 days ago for the unrelated "Iniciar Sesión Rápida" leg) — confirmed by `events.md`'s own explicit text, "'Continuar Día 2' is the identical CTA and underlying computed logic as `home.md` §3.6 — surfaced here as a second, equally valid entry point into the same single global selling state, not a separate implementation." Cloned only `185:699` itself (nav-diverge only, content unchanged) → `1017:3313`; its Button and "Hoy" tab rewired to point at `658:2766` instead of `162:1492`. `988:3234` rewired from `185:699` → `1017:3313`. No new selling/receipt chain built — `658:2766`'s own already-correct downstream (`936:3110`/`936:3120`/`936:3125`, QR-bearing) reused directly.

3. **A misclassified caller found during the first fix's own remediation.** `ux-critic`'s elevated-scrutiny review of items 1–2 (above) caught that `988:3236` (the Eventos-created-event confirmation screen's own Inventario tab, two hops into an unbranched cold-start→schedule→confirm chain that never touches inventory) had been assigned to the post-registration bucket in item 1's fix — contradicting `988:3196`'s own NavBar one hop earlier in the identical chain, which correctly showed "sin registrar." **Fix, after two blocked attempts** (see `company/infrastructure-decisions.md` ID001's sixth recorded instance): `988:3236` repointed back to `981:620`. Two adjacent NavBars in the same chain (`988:3224`, `988:3228`) were audited and found fully inert (`reactionCount: 0` on all tabs, not selectively mis-wired) — left unchanged, flagged as an open `events.md` §3.9 spec question (tap-liveness during the save write) rather than resolved unilaterally.

**Two pre-existing, out-of-scope findings surfaced and logged, not fixed this round** (`product/02-ux/ux-critic-findings.md`): **INV-MF-MAJ1** now confirmed present on both `981:620` and `1014:3292` too (same template-lineage gap as `46:29`/`955:3697`, not introduced by this round). **INV-MF-MAJ2** (new) — `988:3234`'s EventCard reads "empieza en 3 días" but its own destination chain shows "Día 1 · 5 ventas · $610 ya vendidas" — a narrative contradiction pre-existing on the untouched original `185:699`/`162:1905` too, classified Major by `ux-critic`, not part of this round's two named defects.

**Verification, full chain.** `ux-critic`: two passes (elevated-scrutiny on items 1–2, clean aside from the `988:3236` finding fixed in item 3; verification pass on item 3's fix, clean at the content/structure level, reaction-wiring claim explicitly named as outside its tool tier per ID004). `reviewer`: clean, no Blockers, no Important findings — confirmed the `658:2766` reuse is directly grounded in `events.md`'s own explicit "same single global selling state" text rather than merely uncontradicted by it, confirmed D5/D15/D17/D33/D40 consistency, confirmed `1017:3313` is a correctly-classified nav-diverge-only clone. Main independently live-click-verified both hops end to end on fresh pages: `988:3229`→Inventario→`981:620`, all three rows "sin registrar" ✓; `988:3229`→EventCard→`1017:3313` ("Día 1 · 5 ventas · $610 · Continuar Día 2") →"Continuar Día 2"→`658:2766` (Paid-tier Día 2 continuation) ✓.

**Status:** fully closed — all three round-2 items built, `ux-critic`-clean, `reviewer`-clean, wiring-verified live end to end by Main. The standing next step is a fresh, isolated `merchant-user-tester` rerun of the full Paid journey from the beginning (round 3), per the project's "rerun from the beginning, never resume" rule — this is now the second such rerun for this family, since round 1's own rerun is what surfaced these three items.

### 2026-08-12 — `184:1503` family reconvergence, round 3: the literal onboarding-entry path (never walked before) surfaces a picker-overlay reconvergence and a wrong content-diverge redirect

Round 2's own rerun (above) walked the family's own dispatched-shortcut nodes and came back clean — but a fresh `merchant-user-tester` walk starting from the *actual* onboarding entry point (`node-id=162:320`, Bienvenida → "Activar plan de pago" → ... → Home, never previously walked end-to-end by any tester this session) surfaced a severe cluster: a stale non-zero sales total, phantom unregistered products (Chalecos/Blusas/Gorras), Playeras flipping 20→0 disponibles, a sale that didn't move the day's total, and Configuración stuck on "Tu plan: Gratis" despite Paid activation at the start of the same walk.

**Ground-truth trace, before any fix (per standing discipline):** the root cause was not a regression of anything fixed in rounds 1–2. `988:3202` (the onboarding-dedicated Eventos-leg form, correctly cloned earlier) requires a merchant to actually select "Lugar" (it starts unset) — but doing so opened `162:1829`, a **shared, un-cloned** Lugar-picker overlay whose rows hardcoded `NAVIGATE` into the legacy chain `162:1843`→`162:1896`→`162:1900`→`185:699`→`162:1492`→`162:1500`, the exact Free-tier/unrelated-older-persona chain this family's other legs had already been fixed away from. The Tipo picker (`162:1862`) had the identical problem in the mirror direction. Neither picker overlay had ever been cloned, because no prior round's walk had actually needed to select Lugar through this specific leg.

1. **Fix — Lugar/Tipo picker-overlay reconvergence.** Cloned `162:1829`→`1070:3358` (Lugar picker) and `162:1862`→`1070:3372` (Tipo picker), both routing into a new dedicated "Lugar seleccionado" frame `1070:3335` (clone of `162:1843`) whose own "Guardar evento" leads to the existing `988:3225`, not the legacy chain. `988:3202`'s own FieldBoxes rewired to the two new clones. Caller-audit confirmed the shared originals' pre-existing callers (3 each) unaffected — first attempt at this fix was lost to an infrastructure/API-connection error mid-work, not a logical block; retried with an inspect-first step that found and completed the partially-created nodes rather than duplicating them.

2. **`ux-critic` caught a real Blocker in the fix's own first pass (`FIX1-B1`):** `1070:3335`, cloned faithfully from `162:1843`, carried that source's raw content (Tipo="Elegir tipo," unset) instead of being adapted for its real context — `988:3202` has Tipo="Bazar" pre-set, so a merchant arriving via this leg would see her prior selection appear silently discarded. **Fixed:** `1070:3335`'s Tipo field corrected to "Bazar." Two wiring boundaries `ux-critic` couldn't check itself (no reaction-read access, ID004) were closed via fresh `node.reactions` reads: "Guardar evento" confirmed live despite its muted styling (a document-wide button convention, not a disabled state — audited across all 7 sibling form-variants); the Tipo FieldBox confirmed live, not dead. One new pre-existing, document-wide Minor logged (`INV-MF-MIN2`, `ux-critic-findings.md`): reopening the Tipo picker from an already-correct frame reverts Lugar's display — confirmed byte-for-byte identical on the untouched original flow (`162:1877`), not a regression, correctly left unfixed.

3. **Separately, five items investigated in the merchandise-registration leg** (independent of the Eventos-leg fix above, dispatched in parallel): `827:3061`'s "Continuar" (zero reactions) confirmed spec-correct per `onboarding.md` §3.5b's actual gate text, not a defect. `981:620`'s "Hoy" tab was a `URL`-type reaction (kicked the merchant to a raw Figma link) — fixed to `NODE`/`NAVIGATE`→`184:1503`. `651:2679` ("+Agregar otro producto" at 2 committed lines) was dead — fixed with a new 3-line content-diverge clone `1076:6261`, closing a second defect in the same pass (`984:3129`'s static 3-product summary text, previously unreachable via any live path, is now genuinely reachable). "Terminar después" (`984:3137`)→`668:526` was an empty-state Inventario screen wrong for a merchant who just registered real merchandise, with a fully dead NavBar — `668:526` caller-audited (4 legitimate pre-registration callers preserved), `984:3137` redirected, `668:526`'s own NavBar given working reactions.

4. **`reviewer` caught a second real Blocker:** item 3's `984:3137` redirect landed on `1014:3292` — the *wrong* content-diverge sibling. `1014:3292` is the fully-registered Catalog view (§3.4); but `inventory.md` §3.17 requires "Terminar después" to land on §3.5's pending-tag-work content instead (a status line showing remaining untagged units, "Continuar etiquetando" as primary CTA) — and per `ux-critic-findings.md`'s own prior note, the Inventario Catalog-view pending-tag card is the *sole* discoverable path back to resuming deferred tagging anywhere in the product, so the wrong redirect made 7 still-untagged units invisible and unreachable. **Fixed:** new clone `1117:3443` (from the existing `593:526` §3.5 template, correct quantities/copy confirmed against the live queue's own "Faltan 7 de 10"), `984:3137` redirected there, its own two buttons traced and wired to this leg's own destinations (`984:3129`, `651:2646`). A non-blocking Suggestion in the same area (stale `Price Pijama`/`Price Sudadera`/`Price Calcetines` internal layer names, pre-dating the Luna Mercado data swap) was fixed in the same pass at no extra scope cost.

**Verification, full chain.** `ux-critic`: three passes across the round (initial review catching `FIX1-B1`; verification of that fix, clean; independent review of the merchandise-registration items, clean). `reviewer`: one pass catching the `984:3137` misdirect; one verification pass confirming the fix's content is §3.17-compliant (citing the spec text directly, not the builder's own citation) while explicitly naming the wiring claims as outside its tool tier this dispatch (no `chrome-devtools-mcp` access that session). Main closed every remaining wiring boundary via direct `chrome-devtools-mcp` click-through on fresh pages: `988:3202`→Lugar picker→"Plaza Norte"→`1070:3335` (Lugar AND Tipo both genuinely shown set) ✓ →"Guardar evento"→`988:3225` (dedicated chain, not legacy) ✓; `984:3129`("Faltan 7 de 10")→"Terminar después"→`1117:3443` (correct pending-tag content, "7" matching the live queue) ✓ →"Continuar etiquetando"→`984:3129` ✓; `1117:3443`→"Registrar mercancía"→`651:2646` ✓.

**Status:** fully closed — every item built, reviewed clean by both `ux-critic` and `reviewer`, every reaction-level claim independently live-verified by Main (not left as an unverified boundary). Standing next step: a fourth, isolated `merchant-user-tester` rerun of the complete Paid journey from the actual onboarding entry point (`162:320`), per the "rerun from the beginning, never resume" rule — this is the leg that had never been walked end-to-end before round 3 found it broken, so this rerun is the first real confirmation that the literal onboarding entry now holds up, not a repeat of an already-passing path.
