...
**RFC 0007 §3's own test, applied at the moment it said it would be.** RFC 0007 §3 named this exact field and deferred it explicitly: *"if it's ever wanted, it gets its own explicit Product Decision at that time... the same restraint D26 already applied to `Sale.claimToken`."* Note precisely what was deferred to — "its own explicit **Product Decision**," not "its own RFC." `Sale.claimToken` (D26) is the load-bearing precedent: a new field on an existing Selling aggregate, referencing/enabling a cross-context (Loyalty-claim) consumption pattern, logged as a plain decision-log entry, never an RFC. `Sale.performedByMembershipId` is architecturally the same shape, one context over (Identity instead of Loyalty-claim).
