...
**Corrected on one narrower point, `decision-log.md` D66 (2026-09-13, Stage 7 Phase 2 Gap Analysis).** This section's "zero schema change" claim reasoned correctly about *uniqueness/relationship* constraints (nothing new needed there — unchanged by D66) but never separately reckoned with *attribution*: resolving "which Session belongs to which Membership" at all requires a stored `Session.openedByMembershipId` field, the same class of need `Sale.performedByMembershipId` (D58) already serves one level down. D66 adds this field to `domain-model.md`'s `Session` entry and promotes "one active Session per Membership" from an already-enforced-but-undocumented invariant to a named one. This RFC's substantive ruling (no new aggregate, no new relationship, no new Event/Business-level constraint) is otherwise entirely unaffected.
