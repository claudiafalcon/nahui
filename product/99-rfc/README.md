...
- `0010-event-scoped-inventory-allocation-commitment-lifecycle-correction.md` — Proposed. Corrects RFC 0009 §3(b): a confirmed production bug where manual-mode allocation's `quantityRemaining` counter never touched `InventoryUnit.status`, letting Quick Sale or another Event consume "reserved" stock. Replaces the counter with `commitAllocation()`, a real row-level FIFO reservation generalizing the NFC mechanism to manual mode; separates soft `quantityPlanned` intent from hard commitment; retires `quantityRemaining` as a stored field (now derived); splits Event-close release behavior by `unitSource` (manual auto-releases, NFC-tagged stays merchant-confirmed, per the already-Approved `events.md` §3.16/§3.25). One Product Owner decision still open (Open Item 4).
- `0011-event-assignment.md` — Proposed. `EventAssignment` as a new Selling-context aggregate root letting an OWNER pre-assign staff to Events; scheduling-conflict handling is warn-with-override. Knowingly reopens Q24/Q25 item 3's settled "no OWNER pre-assignment" decision at the Product Owner's own direction. Two Product Owner items still open (Open Items 1–2).
