...
- `0012-auth-identity-multi-method.md` — Proposed. Introduces `AuthIdentity` (new Identity-context aggregate root): one `User` may hold multiple linked, independently-verifiable authentication credentials (phone today; email/Google/Apple modeled but not exposed), decoupling `User`'s identity from any single credential. `User.phone`/`phoneVerifiedAt` removed, replaced by a pointer to `AuthIdentity`; the Owner-creation invariant (D44) and Invitation-acceptance invariant (D56) both amended to resolve through `AuthIdentity` (guarantees unchanged, only the field path is corrected). `Invitation` deliberately stays phone-specific (a reasoned choice, not a default). New AuthIdentity-resolution and linking/duplicate-prevention invariants make "adding a method never creates a second business identity" structural. Confirms zero UX change to `authentication.md`'s already-Approved phone-OTP flow — a data-layer rewire underneath an unmodified spec, same posture as RFC 0007/0008. `knowledge-mentor`-confirmed against Supabase Auth's own native multi-provider `identities` model. Triggered by the Product Owner's direction not to let phone remain `User`'s fundamental identity, prompted by field feedback that a new Mexican government mobile-line-registration policy is making some prospective merchants reluctant to register a phone number at all — pending `reviewer`.
