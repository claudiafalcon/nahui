...
Status: Draft — first version built 2026-08-08 against `decision-log.md` D34/D35/D37. **`ux-critic` round 1: 2 Major + 3 Minor found (a self-contradictory "No, gracias" button-weight annotation; a missing decline affordance and trust-footer omission on §3.6; a concurrent-claim race not named) — all fixed.** `ux-critic` verification pass: the 5 original findings confirmed resolved, no regressions; pending `reviewer`'s Foundation-consistency pass.
...
- **The QR's actual rendering on the merchant's Digital Receipt** — `home.md` §3.8f currently shows only a literal, mechanism-free placeholder ("algún día vas a poder registrar aquí tu compra"), not a live QR yet. This document specifies the destination such a QR would resolve to; it does not itself add the QR to `home.md`, which remains that document's own future amendment.
...
2. **Architect Decision candidate.** Whether a Claim Token generated while `loyaltyEnabled = true` remains resolvable if the merchant later toggles `loyaltyEnabled = false` (via `settings.md`'s "Desactivar clientes frecuentes") before the customer ever scans it. Not addressed by D34, D35, or D37. This document's §2 doesn't check `loyaltyEnabled` at resolution time at all — flagged here rather than silently assumed either way.
